...
public class delete_api_v1_admintravelservice_admintravel_1234567890123456789012345678901234567890123456_9a713adb_601 {
    ...
    @Test
    public void test_negative_DELETE_601_46() throws Exception {
        // Record test execution for fault detection tracking
        es.us.isa.restest.analysis.FaultDetectionTracker.getInstance()
            .recordTestCase(this.getClass().getName(), "test_negative_DELETE_601_46");

        // 🔧 Test Case Enhancer: Capture test metadata
        es.us.isa.restest.enhancer.TestResultCapture.setTestMetadata(
            "/api/v1/admintravelservice/admintravel/1234567890123456789012345678901234567890123456789012345678901234567890", "DELETE", 
        ...
        Allure.label("subSuite", "test_negative_DELETE_601_46");

        // 🚨 NEGATIVE TEST METADATA
        Allure.parameter("🚨 Test Type", "NEGATIVE (Invalid Input Testing)");
        Allure.parameter("🔴 Invalid Parameters", "tripId=1234567890123456789012345678901234567890123456789012345678901234567890");
        Allure.description("**⚠️ This is a NEGATIVE test case with intentionally invalid inputs.**\n\n" +
                         ...
                         "tripId=1234567890123456789012345678901234567890123456789012345678901234567890\n\n" +
                         "This test expects a 4XX or 5XX error response. If it returns 2XX, the test will FAIL.");
        es.us.isa.restest.enhancer.TestResultCapture.addInvalidParameter("tripId=1234567890123456789012345678901234567890123456789012345678901234567890");
        ...
        // Step 1: ts-admin-travel-service DELETE /api/v1/admintravelservice/admintravel/1234567890123456789012345678901234567890123456789012345678901234567890 [expect not 200]
        ...
        try {
            Allure.step("Step 1: ts-admin-travel-service DELETE /api/v1/admintravelservice/admintravel/1234567890123456789012345678901234567890123456789012345678901234567890 [expect not 200]", () -> {
                Allure.parameter("🏢 Service", "ts-admin-travel-service");
                Allure.parameter("📡 HTTP Method", "DELETE");
                Allure.parameter("🔗 Endpoint", "/api/v1/admintravelservice/admintravel/1234567890123456789012345678901234567890123456789012345678901234567890");
                Allure.parameter("🎯 Expected Status", 200);
                ...
                Allure.description("🎯 **Testing**: ts-admin-travel-service\n" +
                                 "📡 **Method**: DELETE\n" +
                                 "🔗 **Path**: /api/v1/admintravelservice/admintravel/1234567890123456789012345678901234567890123456789012345678901234567890\n" +
                                 "🎯 **Expected**: 200\n" +
                ...
                if (!shouldSkip) {
                    System.out.println("▶️ EXECUTING: Step 1: ts-admin-travel-service DELETE /api/v1/admintravelservice/admintravel/1234567890123456789012345678901234567890123456789012345678901234567890 [expect not 200] (dependency analysis passed)");
                    ...
                    try {
                        ...
                        stepResponse1 = req.when().delete("/api/v1/admintravelservice/admintravel/1234567890123456789012345678901234567890123456789012345678901234567890")
                               .then().log().ifValidationFails()
                               .extract().response();
                        ...
                        invalidParams.put("tripId", "1234567890123456789012345678901234567890123456789012345678901234567890");
                            ...
                            try {
                                ...
                                es.us.isa.restest.generators.ZeroShotLLMGenerator.ValidationResult validationResult = 
                                    llmValidator.validateNegativeTestResponse(
                                        ...
                                        "DELETE",
                                        "/api/v1/admintravelservice/admintravel/1234567890123456789012345678901234567890123456789012345678901234567890",
                                        invalidParams
                                    );
                                    ...
                                    if (dataObj.optBoolean("injected", false)) {
                                        ...
                                        if (!detectedFaultName.isEmpty()) {
                                            es.us.isa.restest.analysis.FaultDetectionTracker.getInstance().recordDetectedFault(
                                                ...
                                                "test_negative_DELETE_601_46",
                                                System.currentTimeMillis(),
                                                faultCheckBody
                                            );
                                            System.out.println("🔍 FAULT DETECTED: " + detectedFaultName + " in test: test_negative_DELETE_601_46");
                                        }
                                    }
                                }
                            }
                        ...
                        System.out.println("✅ Step 1: ts-admin-travel-service DELETE /api/v1/admintravelservice/admintravel/1234567890123456789012345678901234567890123456789012345678901234567890 [expect not 200] - SUCCESS");
                        ...
                        try {
                            ...
                            attachJaegerTrace("ts-admin-travel-service", "DELETE", "/api/v1/admintravelservice/admintravel/1234567890123456789012345678901234567890123456789012345678901234567890", requestStartMicros, allStepParameters, false);
                        } catch (Exception e) {
                            Allure.parameter("🎯 Result", "✅ SUCCESS (response capture failed)");
                        }
                    } catch (Throwable t) {
                        stepResults.put(1, false);
                        System.out.println("❌ Step 1: ts-admin-travel-service DELETE /api/v1/admintravelservice/admintravel/1234567890123456789012345678901234567890123456789012345678901234567890 [expect not 200] - FAILED: " + t.getMessage());
                                    ...
                                    if (dataValue instanceof org.json.JSONObject) {
                                        ...
                                        if (dataObj.optBoolean("injected", false)) {
                                            ...
                                            if (!detectedFaultName.isEmpty()) {
                                                ...
                                                System.out.println("🔍 FAULT DETECTED (in failed test): " + detectedFaultName + " in test: test_negative_DELETE_601_46");
                                            }
                                        }
                                    }
                                }
                        ...
                        Allure.parameter("🔗 Failed Endpoint", "/api/v1/admintravelservice/admintravel/1234567890123456789012345678901234567890123456789012345678901234567890");
                        ...
                        errorDetails.append("=====================================\n\n");
                        errorDetails.append("📋 Step: Step 1: ts-admin-travel-service DELETE /api/v1/admintravelservice/admintravel/1234567890123456789012345678901234567890123456789012345678901234567890 [expect not 200]\n");
                        errorDetails.append("🏢 Service: ts-admin-travel-service\n");
                        errorDetails.append("📡 Method: DELETE\n");
                        errorDetails.append("🔗 Endpoint: /api/v1/admintravelservice/admintravel/1234567890123456789012345678901234567890123456789012345678901234567890\n");
                        ...
                        // ⏱️ Wait longer for trace propagation to Jaeger (increased delay)
                        try { Thread.sleep(3000); } catch (InterruptedException ie) { Thread.currentThread().interrupt(); }
                        attachJaegerTrace("ts-admin-travel-service", "DELETE", "/api/v1/admintravelservice/admintravel/1234567890123456789012345678901234567890123456789012345678901234567890", requestStartMicros, allStepParameters, true);
                        
                        // 🔥 CRITICAL: Throw exception to mark step as FAILED (red arrow) in Allure
                        throw new RuntimeException("Step 1: ts-admin-travel-service DELETE /api/v1/admintravelservice/admintravel/1234567890123456789012345678901234567890123456789012345678901234567890 [expect not 200] failed: " + failureReason + " (" + errorType + ")", t);
                    }
                } else {
                    // ⏭️ SKIP: Clean skip reporting with proper Allure status
                    System.out.println("⏭️ SKIPPING: Step 1: ts-admin-travel-service DELETE /api/v1/admintravelservice/admintravel/1234567890123456789012345678901234567890123456789012345678901234567890 [expect not 200] - " + skipReason);
                    stepResults.put(1, false);
            ...
            if (stepException instanceof RuntimeException && stepException.getMessage().startsWith("Step failed:")) {
                ...
                System.out.println("⚠️ Step wrapper failed for Step 1: ts-admin-travel-service DELETE /api/v1/admintravelservice/admintravel/1234567890123456789012345678901234567890123456789012345678901234567890 [expect not 200]: " + stepException.getMessage());
                stepResults.put(1, false);
            }
        }
        ...
        Allure.label("story", "test_negative_DELETE_601_46");
        Allure.description("Microservice test scenario with " + totalSteps + " steps.");
        
        System.out.println("=== SCENARIO RESULT ===");
        System.out.println("Scenario: test_negative_DELETE_601_46");
