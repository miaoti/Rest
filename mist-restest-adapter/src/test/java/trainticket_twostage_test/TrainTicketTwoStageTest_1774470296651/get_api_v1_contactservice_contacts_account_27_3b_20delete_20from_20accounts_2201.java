...
public class get_api_v1_contactservice_contacts_account_27_3b_20delete_20from_20accounts_2201 {
    ...
    public void test_negative_GET_2201_46() throws Exception {
        ...
        Allure.parameter("🔴 Invalid Parameters", "accountId='; DELETE FROM accounts--");
        Allure.description("**⚠️ This is a NEGATIVE test case with intentionally invalid inputs.**\n\n" +
                         ...
                         "accountId='; DELETE FROM accounts--\n\n" +
                         "This test expects a 4XX or 5XX error response. If it returns 2XX, the test will FAIL.");
        es.us.isa.restest.enhancer.TestResultCapture.addInvalidParameter("accountId='; DELETE FROM accounts--");
        ...
        // Step 1: ts-contacts-service GET /api/v1/contactservice/contacts/account/%27%3B%20DELETE%20FROM%20accounts-- [expect not 200]
        ...
        try {
            Allure.step("Step 1: ts-contacts-service GET /api/v1/contactservice/contacts/account/%27%3B%20DELETE%20FROM%20accounts-- [expect not 200]", () -> {
                Allure.parameter("🏢 Service", "ts-contacts-service");
                Allure.parameter("📡 HTTP Method", "GET");
                Allure.parameter("🔗 Endpoint", "/api/v1/contactservice/contacts/account/%27%3B%20DELETE%20FROM%20accounts--");
                Allure.parameter("🎯 Expected Status", 200);
                ...
                Allure.description("🎯 **Testing**: ts-contacts-service\n" +
                                 "📡 **Method**: GET\n" +
                                 "🔗 **Path**: /api/v1/contactservice/contacts/account/%27%3B%20DELETE%20FROM%20accounts--\n" +
                                 "🎯 **Expected**: 200\n" +
                ...
                if (!shouldSkip) {
                    System.out.println("▶️ EXECUTING: Step 1: ts-contacts-service GET /api/v1/contactservice/contacts/account/%27%3B%20DELETE%20FROM%20accounts-- [expect not 200] (dependency analysis passed)");
                    ...
                    try {
                        ...
                        stepResponse1 = req.when().get("/api/v1/contactservice/contacts/account/%27%3B%20DELETE%20FROM%20accounts--")
                               .then().log().ifValidationFails()
                               .extract().response();
                        ...
                        invalidParams.put("accountId", "'; DELETE FROM accounts--");
                            ...
                            try {
                                ...
                                if (softErrorCacheEnabled) {
                                    ...
                                    validationResult = llmValidator.validateNegativeResponseWithCache(
                                        ...
                                        "ts-contacts-service",
                                        "GET",
                                        "/api/v1/contactservice/contacts/account/%27%3B%20DELETE%20FROM%20accounts--",
                                        invalidParams,
                                        cache
                                    );
                                } else {
                                    ...
                                    if (dataObj.optBoolean("injected", false)) {
                                        ...
                                        if (!detectedFaultName.isEmpty()) {
                                            es.us.isa.restest.analysis.FaultDetectionTracker.getInstance().recordDetectedFault(
                                                ...
                                                "test_negative_GET_2201_46",
                                                System.currentTimeMillis(),
                                                faultCheckBody
                                            );
                                            System.out.println("🔍 FAULT DETECTED: " + detectedFaultName + " in test: test_negative_GET_2201_46");
                                        }
                                    }
                                }
                            }
                        ...
                        System.out.println("✅ Step 1: ts-contacts-service GET /api/v1/contactservice/contacts/account/%27%3B%20DELETE%20FROM%20accounts-- [expect not 200] - SUCCESS");
                        ...
                        try {
                            ...
                            attachJaegerTrace("ts-contacts-service", "GET", "/api/v1/contactservice/contacts/account/%27%3B%20DELETE%20FROM%20accounts--", requestStartMicros, allStepParameters, false);
                        } catch (Exception e) {
                            Allure.parameter("🎯 Result", "✅ SUCCESS (response capture failed)");
                        }
                    } catch (Throwable t) {
                        stepResults.put(1, false);
                        System.out.println("❌ Step 1: ts-contacts-service GET /api/v1/contactservice/contacts/account/%27%3B%20DELETE%20FROM%20accounts-- [expect not 200] - FAILED: " + t.getMessage());
                                    ...
                                    if (dataValue instanceof org.json.JSONObject) {
                                        ...
                                        if (dataObj.optBoolean("injected", false)) {
                                            ...
                                            if (!detectedFaultName.isEmpty()) {
                                                ...
                                                System.out.println("🔍 FAULT DETECTED (in failed test): " + detectedFaultName + " in test: test_negative_GET_2201_46");
                                            }
                                        }
                                    }
                                }
                        ...
                        Allure.parameter("🔗 Failed Endpoint", "/api/v1/contactservice/contacts/account/%27%3B%20DELETE%20FROM%20accounts--");
                        ...
                        errorDetails.append("=====================================\n\n");
                        errorDetails.append("📋 Step: Step 1: ts-contacts-service GET /api/v1/contactservice/contacts/account/%27%3B%20DELETE%20FROM%20accounts-- [expect not 200]\n");
                        errorDetails.append("🏢 Service: ts-contacts-service\n");
                        errorDetails.append("📡 Method: GET\n");
                        errorDetails.append("🔗 Endpoint: /api/v1/contactservice/contacts/account/%27%3B%20DELETE%20FROM%20accounts--\n");
                        ...
                        // ⏱️ Wait longer for trace propagation to Jaeger (increased delay)
                        try { Thread.sleep(3000); } catch (InterruptedException ie) { Thread.currentThread().interrupt(); }
                        attachJaegerTrace("ts-contacts-service", "GET", "/api/v1/contactservice/contacts/account/%27%3B%20DELETE%20FROM%20accounts--", requestStartMicros, allStepParameters, true);
                        
                        // 🔥 CRITICAL: Throw exception to mark step as FAILED (red arrow) in Allure
                        throw new RuntimeException("Step 1: ts-contacts-service GET /api/v1/contactservice/contacts/account/%27%3B%20DELETE%20FROM%20accounts-- [expect not 200] failed: " + failureReason + " (" + errorType + ")", t);
                    }
                } else {
                    // ⏭️ SKIP: Clean skip reporting with proper Allure status
                    System.out.println("⏭️ SKIPPING: Step 1: ts-contacts-service GET /api/v1/contactservice/contacts/account/%27%3B%20DELETE%20FROM%20accounts-- [expect not 200] - " + skipReason);
                    stepResults.put(1, false);
            ...
            if (stepException instanceof RuntimeException && stepException.getMessage().startsWith("Step failed:")) {
                ...
                System.out.println("⚠️ Step wrapper failed for Step 1: ts-contacts-service GET /api/v1/contactservice/contacts/account/%27%3B%20DELETE%20FROM%20accounts-- [expect not 200]: " + stepException.getMessage());
                stepResults.put(1, false);
            }
        }
        ...
        Allure.label("story", "test_negative_GET_2201_46");
        Allure.description("Microservice test scenario with " + totalSteps + " steps.");
        
        System.out.println("=== SCENARIO RESULT ===");
        System.out.println("Scenario: test_negative_GET_2201_46");
