...
@Epic("Baseline Tests")
@Feature("ts-admin-order-service")
@io.qameta.allure.junit4.DisplayName("Flow_Scenario_1 | GET /api/v1/adminorderservice/adminorder")
public class Flow_Scenario_1 {
    ...
    public void test_positive_flow_S1_v1() throws Exception {
        ...
        Allure.label("parentSuite", "ts-admin-order-service");
        ...
        // Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 200]
        es.us.isa.restest.enhancer.TestResultCapture.setStepMetadata(
            1, "/api/v1/adminorderservice/adminorder", "GET", 
            "ts-admin-order-service", false);
        ...
        try {
            Allure.step("Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 200]", () -> {
                ...
                if (!shouldSkip) {
                    System.out.println("▶️ EXECUTING: Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 200] (dependency analysis passed)");
                    Response stepResponse1 = null;
                    
                    try {
                        RequestSpecification req = RestAssured.given();
                        req = es.us.isa.restest.auth.MstAuthHandler.applyAuth(req, "/api/v1/adminorderservice/adminorder", __mstOverrideToken, __mstDisableAuth);
                        ...
                        stepResponse1 = req.when().get("/api/v1/adminorderservice/adminorder")
                                    ...
                                    if (dataObj.optBoolean("injected", false)) {
                                        ...
                                        if (!detectedFaultName.isEmpty()) {
                                            es.us.isa.restest.analysis.FaultDetectionTracker.getInstance().recordDetectedFault(
                                                ...
                                                "test_positive_flow_S1_v1",
                                                System.currentTimeMillis(),
                                                faultCheckBody
                                            );
                                            System.out.println("🔍 FAULT DETECTED: " + detectedFaultName + " in test: test_positive_flow_S1_v1");
                                        }
                                    }
                                }
                            }
                        ...
                        System.out.println("✅ Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 200] - SUCCESS");
                        try {
                        ...
                        try {
                            ...
                            attachJaegerTrace("ts-admin-order-service", "GET", "/api/v1/adminorderservice/adminorder", requestStartMicros, allStepParameters, false);
                        } catch (Exception e) {
                            Allure.parameter("🎯 Result", "✅ SUCCESS (response capture failed)");
                        }
                    } catch (Throwable t) {
                        stepResults.put(1, false);
                        System.out.println("❌ Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 200] - FAILED: " + t.getMessage());
                                    ...
                                    if (dataValue instanceof org.json.JSONObject) {
                                        ...
                                        if (dataObj.optBoolean("injected", false)) {
                                            ...
                                            if (!detectedFaultName.isEmpty()) {
                                                ...
                                                System.out.println("🔍 FAULT DETECTED (in failed test): " + detectedFaultName + " in test: test_positive_flow_S1_v1");
                                            }
                                        }
                                    }
                                }
                        ...
                        errorDetails.append("=====================================\n\n");
                        errorDetails.append("📋 Step: Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 200]\n");
                        ...
                        // ⏱️ Wait longer for trace propagation to Jaeger (increased delay)
                        try { Thread.sleep(3000); } catch (InterruptedException ie) { Thread.currentThread().interrupt(); }
                        attachJaegerTrace("ts-admin-order-service", "GET", "/api/v1/adminorderservice/adminorder", requestStartMicros, allStepParameters, true);
                        
                        // 🔥 CRITICAL: Throw exception to mark step as FAILED (red arrow) in Allure
                        throw new RuntimeException("Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 200] failed: " + failureReason + " (" + errorType + ")", t);
                    }
                } else {
                    // ⏭️ SKIP: Clean skip reporting with proper Allure status
                    System.out.println("⏭️ SKIPPING: Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 200] - " + skipReason);
                    stepResults.put(1, false);
            ...
            if (stepException instanceof RuntimeException && stepException.getMessage().startsWith("Step failed:")) {
                ...
                System.out.println("⚠️ Step wrapper failed for Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 200]: " + stepException.getMessage());
                stepResults.put(1, false);
            }
        }
    ...
    @Test
    public void test_positive_flow_S1_v1_explore_200() throws Exception {
        // Record test execution for fault detection tracking
        es.us.isa.restest.analysis.FaultDetectionTracker.getInstance()
            .recordTestCase(this.getClass().getName(), "test_positive_flow_S1_v1_explore_200");
        ...
        // Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 200 (exploration)]
        es.us.isa.restest.enhancer.TestResultCapture.setStepMetadata(
            1, "/api/v1/adminorderservice/adminorder", "GET", 
            "ts-admin-order-service", false);
        ...
        final long requestStartMicros = System.currentTimeMillis() * 1000L;
        try {
            Allure.step("Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 200 (exploration)]", () -> {
                Allure.parameter("🏢 Service", "ts-admin-order-service");
                Allure.parameter("📡 HTTP Method", "GET");
                Allure.parameter("🔗 Endpoint", "/api/v1/adminorderservice/adminorder");
                Allure.parameter("🎯 Expected Status", 200);
                ...
                if (!shouldSkip) {
                    System.out.println("▶️ EXECUTING: Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 200 (exploration)] (dependency analysis passed)");
                    Response stepResponse1 = null;
                    ...
                    try {
                                    ...
                                    if (dataObj.optBoolean("injected", false)) {
                                        ...
                                        if (!detectedFaultName.isEmpty()) {
                                            es.us.isa.restest.analysis.FaultDetectionTracker.getInstance().recordDetectedFault(
                                                detectedFaultName,
                                                this.getClass().getName(),
                                                "test_positive_flow_S1_v1_explore_200",
                                                System.currentTimeMillis(),
                                                faultCheckBody
                                            );
                                            System.out.println("🔍 FAULT DETECTED: " + detectedFaultName + " in test: test_positive_flow_S1_v1_explore_200");
                                        }
                                    }
                                }
                            }
                        ...
                        stepResults.put(1, true);
                        System.out.println("✅ Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 200 (exploration)] - SUCCESS");
                        try {
                            String fullBody1 = stepResponse1.getBody().asString();
                            if (fullBody1 != null && !fullBody1.isEmpty()) {
                                capturedOutputs.put(1, fullBody1);
                        ...
                        stepResults.put(1, false);
                        System.out.println("❌ Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 200 (exploration)] - FAILED: " + t.getMessage());
                                    ...
                                    if (dataValue instanceof org.json.JSONObject) {
                                        ...
                                        if (dataObj.optBoolean("injected", false)) {
                                            ...
                                            if (!detectedFaultName.isEmpty()) {
                                                ...
                                                System.out.println("🔍 FAULT DETECTED (in failed test): " + detectedFaultName + " in test: test_positive_flow_S1_v1_explore_200");
                                            }
                                        }
                                    }
                                }
                        ...
                        errorDetails.append("❌ STEP FAILURE ANALYSIS\n");
                        errorDetails.append("=====================================\n\n");
                        errorDetails.append("📋 Step: Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 200 (exploration)]\n");
                        errorDetails.append("🏢 Service: ts-admin-order-service\n");
                        errorDetails.append("📡 Method: GET\n");
                        errorDetails.append("🔗 Endpoint: /api/v1/adminorderservice/adminorder\n");
                        errorDetails.append("💥 Error Type: ").append(errorType).append("\n");
                        ...
                        // 🔥 CRITICAL: Throw exception to mark step as FAILED (red arrow) in Allure
                        throw new RuntimeException("Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 200 (exploration)] failed: " + failureReason + " (" + errorType + ")", t);
                    }
                } else {
                    // ⏭️ SKIP: Clean skip reporting with proper Allure status
                    System.out.println("⏭️ SKIPPING: Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 200 (exploration)] - " + skipReason);
                    stepResults.put(1, false);
            ...
            if (stepException instanceof RuntimeException && stepException.getMessage().startsWith("Step failed:")) {
                ...
                // Unexpected wrapper failure
                System.out.println("⚠️ Step wrapper failed for Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 200 (exploration)]: " + stepException.getMessage());
                stepResults.put(1, false);
            }
        }
        } // end per-step scope block
        ...
        System.out.println("=== SCENARIO RESULT ===");
        System.out.println("Scenario: test_positive_flow_S1_v1_explore_200");
        System.out.println("Total Steps: " + totalSteps);
        System.out.println("Successful: " + successfulSteps);
        System.out.println("Failed: " + failedSteps);
        System.out.println("Login Status: " + (loginSucceeded.get() ? "SUCCESS" : "FAILED"));
    ...
    @Test
    public void test_positive_flow_S1_v1_explore_400() throws Exception {
        // ═══════════════════════════════════════════════════════════════════════
        ...
        Allure.addAttachment("📝 Original Failure", "text/plain", "Original Status: 0\nOriginal Response: \nEnhanced Parameters: {orderId=5ad7750b-9e67-4b8d-8d3b-5f1c3e7a8d9b}");

        // Record test execution for fault detection tracking
        es.us.isa.restest.analysis.FaultDetectionTracker.getInstance()
            .recordTestCase(this.getClass().getName(), "test_positive_flow_S1_v1_explore_400");
        ...
        // Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 400 (exploration)]
        es.us.isa.restest.enhancer.TestResultCapture.setStepMetadata(
            1, "/api/v1/adminorderservice/adminorder", "GET", 
            "ts-admin-order-service", false);
        ...
        final long requestStartMicros = System.currentTimeMillis() * 1000L;
        try {
            Allure.step("Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 400 (exploration)]", () -> {
                Allure.parameter("🏢 Service", "ts-admin-order-service");
                Allure.parameter("📡 HTTP Method", "GET");
                Allure.parameter("🔗 Endpoint", "/api/v1/adminorderservice/adminorder");
                Allure.parameter("🎯 Expected Status", 200);
                ...
                if (!shouldSkip) {
                    System.out.println("▶️ EXECUTING: Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 400 (exploration)] (dependency analysis passed)");
                    Response stepResponse1 = null;
                    ...
                    try {
                                    ...
                                    if (dataObj.optBoolean("injected", false)) {
                                        ...
                                        if (!detectedFaultName.isEmpty()) {
                                            es.us.isa.restest.analysis.FaultDetectionTracker.getInstance().recordDetectedFault(
                                                detectedFaultName,
                                                this.getClass().getName(),
                                                "test_positive_flow_S1_v1_explore_400",
                                                System.currentTimeMillis(),
                                                faultCheckBody
                                            );
                                            System.out.println("🔍 FAULT DETECTED: " + detectedFaultName + " in test: test_positive_flow_S1_v1_explore_400");
                                        }
                                    }
                                }
                            }
                        ...
                        stepResults.put(1, true);
                        System.out.println("✅ Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 400 (exploration)] - SUCCESS");
                        try {
                            String fullBody1 = stepResponse1.getBody().asString();
                            if (fullBody1 != null && !fullBody1.isEmpty()) {
                                capturedOutputs.put(1, fullBody1);
                        ...
                        stepResults.put(1, false);
                        System.out.println("❌ Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 400 (exploration)] - FAILED: " + t.getMessage());
                                    ...
                                    if (dataValue instanceof org.json.JSONObject) {
                                        ...
                                        if (dataObj.optBoolean("injected", false)) {
                                            ...
                                            if (!detectedFaultName.isEmpty()) {
                                                ...
                                                System.out.println("🔍 FAULT DETECTED (in failed test): " + detectedFaultName + " in test: test_positive_flow_S1_v1_explore_400");
                                            }
                                        }
                                    }
                                }
                        ...
                        errorDetails.append("❌ STEP FAILURE ANALYSIS\n");
                        errorDetails.append("=====================================\n\n");
                        errorDetails.append("📋 Step: Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 400 (exploration)]\n");
                        errorDetails.append("🏢 Service: ts-admin-order-service\n");
                        errorDetails.append("📡 Method: GET\n");
                        errorDetails.append("🔗 Endpoint: /api/v1/adminorderservice/adminorder\n");
                        errorDetails.append("💥 Error Type: ").append(errorType).append("\n");
                        ...
                        // 🔥 CRITICAL: Throw exception to mark step as FAILED (red arrow) in Allure
                        throw new RuntimeException("Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 400 (exploration)] failed: " + failureReason + " (" + errorType + ")", t);
                    }
                } else {
                    // ⏭️ SKIP: Clean skip reporting with proper Allure status
                    System.out.println("⏭️ SKIPPING: Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 400 (exploration)] - " + skipReason);
                    stepResults.put(1, false);
            ...
            if (stepException instanceof RuntimeException && stepException.getMessage().startsWith("Step failed:")) {
                ...
                // Unexpected wrapper failure
                System.out.println("⚠️ Step wrapper failed for Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 400 (exploration)]: " + stepException.getMessage());
                stepResults.put(1, false);
            }
        }
        } // end per-step scope block
        ...
        System.out.println("=== SCENARIO RESULT ===");
        System.out.println("Scenario: test_positive_flow_S1_v1_explore_400");
        System.out.println("Total Steps: " + totalSteps);
        System.out.println("Successful: " + successfulSteps);
        System.out.println("Failed: " + failedSteps);
        System.out.println("Login Status: " + (loginSucceeded.get() ? "SUCCESS" : "FAILED"));
    ...
    @Test
    public void test_positive_flow_S1_v1_explore_401() throws Exception {
        // Record test execution for fault detection tracking
        es.us.isa.restest.analysis.FaultDetectionTracker.getInstance()
            .recordTestCase(this.getClass().getName(), "test_positive_flow_S1_v1_explore_401");
        ...
        // Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 401 (exploration)]
        es.us.isa.restest.enhancer.TestResultCapture.setStepMetadata(
            1, "/api/v1/adminorderservice/adminorder", "GET", 
            "ts-admin-order-service", false);
        ...
        final long requestStartMicros = System.currentTimeMillis() * 1000L;
        try {
            Allure.step("Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 401 (exploration)]", () -> {
                Allure.parameter("🏢 Service", "ts-admin-order-service");
                Allure.parameter("📡 HTTP Method", "GET");
                Allure.parameter("🔗 Endpoint", "/api/v1/adminorderservice/adminorder");
                Allure.parameter("🎯 Expected Status", 200);
                ...
                if (!shouldSkip) {
                    System.out.println("▶️ EXECUTING: Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 401 (exploration)] (dependency analysis passed)");
                    Response stepResponse1 = null;
                    ...
                    try {
                                    ...
                                    if (dataObj.optBoolean("injected", false)) {
                                        ...
                                        if (!detectedFaultName.isEmpty()) {
                                            es.us.isa.restest.analysis.FaultDetectionTracker.getInstance().recordDetectedFault(
                                                detectedFaultName,
                                                this.getClass().getName(),
                                                "test_positive_flow_S1_v1_explore_401",
                                                System.currentTimeMillis(),
                                                faultCheckBody
                                            );
                                            System.out.println("🔍 FAULT DETECTED: " + detectedFaultName + " in test: test_positive_flow_S1_v1_explore_401");
                                        }
                                    }
                                }
                            }
                        ...
                        stepResults.put(1, true);
                        System.out.println("✅ Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 401 (exploration)] - SUCCESS");
                        try {
                            String fullBody1 = stepResponse1.getBody().asString();
                            if (fullBody1 != null && !fullBody1.isEmpty()) {
                                capturedOutputs.put(1, fullBody1);
                        ...
                        stepResults.put(1, false);
                        System.out.println("❌ Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 401 (exploration)] - FAILED: " + t.getMessage());
                                    ...
                                    if (dataValue instanceof org.json.JSONObject) {
                                        ...
                                        if (dataObj.optBoolean("injected", false)) {
                                            ...
                                            if (!detectedFaultName.isEmpty()) {
                                                ...
                                                System.out.println("🔍 FAULT DETECTED (in failed test): " + detectedFaultName + " in test: test_positive_flow_S1_v1_explore_401");
                                            }
                                        }
                                    }
                                }
                        ...
                        errorDetails.append("❌ STEP FAILURE ANALYSIS\n");
                        errorDetails.append("=====================================\n\n");
                        errorDetails.append("📋 Step: Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 401 (exploration)]\n");
                        errorDetails.append("🏢 Service: ts-admin-order-service\n");
                        errorDetails.append("📡 Method: GET\n");
                        errorDetails.append("🔗 Endpoint: /api/v1/adminorderservice/adminorder\n");
                        errorDetails.append("💥 Error Type: ").append(errorType).append("\n");
                        ...
                        // 🔥 CRITICAL: Throw exception to mark step as FAILED (red arrow) in Allure
                        throw new RuntimeException("Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 401 (exploration)] failed: " + failureReason + " (" + errorType + ")", t);
                    }
                } else {
                    // ⏭️ SKIP: Clean skip reporting with proper Allure status
                    System.out.println("⏭️ SKIPPING: Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 401 (exploration)] - " + skipReason);
                    stepResults.put(1, false);
            ...
            if (stepException instanceof RuntimeException && stepException.getMessage().startsWith("Step failed:")) {
                ...
                // Unexpected wrapper failure
                System.out.println("⚠️ Step wrapper failed for Root 1: ts-admin-order-service GET /api/v1/adminorderservice/adminorder [expect 401 (exploration)]: " + stepException.getMessage());
                stepResults.put(1, false);
            }
        }
        } // end per-step scope block
        ...
        System.out.println("=== SCENARIO RESULT ===");
        System.out.println("Scenario: test_positive_flow_S1_v1_explore_401");
        System.out.println("Total Steps: " + totalSteps);
        System.out.println("Successful: " + successfulSteps);
        System.out.println("Failed: " + failedSteps);
        System.out.println("Login Status: " + (loginSucceeded.get() ? "SUCCESS" : "FAILED"));
