...
@Epic("Baseline Tests")
@Feature("ts-station-service")
@io.qameta.allure.junit4.DisplayName("Flow_Scenario_1737 | GET /api/v1/stationservice/stations")
public class Flow_Scenario_1737 {
    ...
    @Test
    public void test_positive_flow_S1737_v1() throws Exception {
        // Record test execution for fault detection tracking
        es.us.isa.restest.analysis.FaultDetectionTracker.getInstance()
            .recordTestCase(this.getClass().getName(), "test_positive_flow_S1737_v1");
        ...
        Allure.label("parentSuite", "ts-station-service");
        ...
        try {
            Allure.step("Root 1: ts-station-service GET /api/v1/stationservice/stations [expect 200]", () -> {
                Allure.parameter("🏢 Service", "ts-station-service");
                Allure.parameter("📡 HTTP Method", "GET");
                Allure.parameter("🔗 Endpoint", "/api/v1/stationservice/stations");
                ...
                if (!shouldSkip) {
                    System.out.println("▶️ EXECUTING: Root 1: ts-station-service GET /api/v1/stationservice/stations [expect 200] (dependency analysis passed)");
                    Response stepResponse1 = null;
                    
                    try {
                        RequestSpecification req = RestAssured.given();
                        req = es.us.isa.restest.auth.MstAuthHandler.applyAuth(req, "/api/v1/stationservice/stations", __mstOverrideToken, __mstDisableAuth);
                        ...
                        stepResponse1 = req.when().get("/api/v1/stationservice/stations")
                                    ...
                                    if (dataObj.optBoolean("injected", false)) {
                                        ...
                                        if (!detectedFaultName.isEmpty()) {
                                            es.us.isa.restest.analysis.FaultDetectionTracker.getInstance().recordDetectedFault(
                                                ...
                                                "test_positive_flow_S1737_v1",
                                                System.currentTimeMillis(),
                                                faultCheckBody
                                            );
                                            System.out.println("🔍 FAULT DETECTED: " + detectedFaultName + " in test: test_positive_flow_S1737_v1");
                                        }
                                    }
                                }
                            }
                        ...
                        System.out.println("✅ Root 1: ts-station-service GET /api/v1/stationservice/stations [expect 200] - SUCCESS");
                        try {
                        ...
                        try {
                            ...
                            attachJaegerTrace("ts-station-service", "GET", "/api/v1/stationservice/stations", requestStartMicros, allStepParameters, false);
                        } catch (Exception e) {
                            Allure.parameter("🎯 Result", "✅ SUCCESS (response capture failed)");
                        }
                    } catch (Throwable t) {
                        stepResults.put(1, false);
                        System.out.println("❌ Root 1: ts-station-service GET /api/v1/stationservice/stations [expect 200] - FAILED: " + t.getMessage());
                                    ...
                                    if (dataValue instanceof org.json.JSONObject) {
                                        ...
                                        if (dataObj.optBoolean("injected", false)) {
                                            ...
                                            if (!detectedFaultName.isEmpty()) {
                                                ...
                                                System.out.println("🔍 FAULT DETECTED (in failed test): " + detectedFaultName + " in test: test_positive_flow_S1737_v1");
                                            }
                                        }
                                    }
                                }
                        ...
                        Allure.parameter("🏢 Failed Service", "ts-station-service");
                        Allure.parameter("📡 Failed Method", "GET");
                        Allure.parameter("🔗 Failed Endpoint", "/api/v1/stationservice/stations");
                        ...
                        errorDetails.append("=====================================\n\n");
                        errorDetails.append("📋 Step: Root 1: ts-station-service GET /api/v1/stationservice/stations [expect 200]\n");
                        errorDetails.append("🏢 Service: ts-station-service\n");
                        errorDetails.append("📡 Method: GET\n");
                        errorDetails.append("🔗 Endpoint: /api/v1/stationservice/stations\n");
                        ...
                        // ⏱️ Wait longer for trace propagation to Jaeger (increased delay)
                        try { Thread.sleep(3000); } catch (InterruptedException ie) { Thread.currentThread().interrupt(); }
                        attachJaegerTrace("ts-station-service", "GET", "/api/v1/stationservice/stations", requestStartMicros, allStepParameters, true);
                        
                        // 🔥 CRITICAL: Throw exception to mark step as FAILED (red arrow) in Allure
                        throw new RuntimeException("Root 1: ts-station-service GET /api/v1/stationservice/stations [expect 200] failed: " + failureReason + " (" + errorType + ")", t);
                    }
                } else {
                    // ⏭️ SKIP: Clean skip reporting with proper Allure status
                    System.out.println("⏭️ SKIPPING: Root 1: ts-station-service GET /api/v1/stationservice/stations [expect 200] - " + skipReason);
                    stepResults.put(1, false);
            ...
            if (stepException instanceof RuntimeException && stepException.getMessage().startsWith("Step failed:")) {
                ...
                System.out.println("⚠️ Step wrapper failed for Root 1: ts-station-service GET /api/v1/stationservice/stations [expect 200]: " + stepException.getMessage());
                stepResults.put(1, false);
            }
        }
        ...
        System.out.println("Scenario: test_positive_flow_S1737_v1");
