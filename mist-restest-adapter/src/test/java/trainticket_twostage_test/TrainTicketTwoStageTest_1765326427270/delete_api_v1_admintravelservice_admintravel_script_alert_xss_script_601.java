...
public class delete_api_v1_admintravelservice_admintravel_script_alert_xss_script_601 {
    ...
    public void test_negative_DELETE_601_22() throws Exception {
        ...
        es.us.isa.restest.enhancer.TestResultCapture.setTestMetadata(
            "/api/v1/admintravelservice/admintravel/<script>alert('XSS')</script>", "DELETE", 
        ...
        // Step 1: ts-admin-travel-service DELETE /api/v1/admintravelservice/admintravel/<script>alert('XSS')</script> [expect not 200]
        ...
        try {
            Allure.step("Step 1: ts-admin-travel-service DELETE /api/v1/admintravelservice/admintravel/<script>alert('XSS')</script> [expect not 200]", () -> {
                Allure.parameter("🏢 Service", "ts-admin-travel-service");
                Allure.parameter("📡 HTTP Method", "DELETE");
                Allure.parameter("🔗 Endpoint", "/api/v1/admintravelservice/admintravel/<script>alert('XSS')</script>");
                Allure.parameter("🎯 Expected Status", 200);
                ...
                Allure.description("🎯 **Testing**: ts-admin-travel-service\n" +
                                 "📡 **Method**: DELETE\n" +
                                 "🔗 **Path**: /api/v1/admintravelservice/admintravel/<script>alert('XSS')</script>\n" +
                                 "🎯 **Expected**: 200\n" +
                ...
                if (!shouldSkip) {
                    System.out.println("▶️ EXECUTING: Step 1: ts-admin-travel-service DELETE /api/v1/admintravelservice/admintravel/<script>alert('XSS')</script> [expect not 200] (dependency analysis passed)");
                    ...
                    try {
                        ...
                        stepResponse1 = req.when().delete("/api/v1/admintravelservice/admintravel/<script>alert('XSS')</script>")
                               .then().log().ifValidationFails()
                               .extract().response();
                            ...
                            try {
                                ...
                                es.us.isa.restest.generators.ZeroShotLLMGenerator.ValidationResult validationResult = 
                                    llmValidator.validateResponse(
                                        ...
                                        "DELETE",
                                        "/api/v1/admintravelservice/admintravel/<script>alert('XSS')</script>"
                                    );
                        ...
                        System.out.println("✅ Step 1: ts-admin-travel-service DELETE /api/v1/admintravelservice/admintravel/<script>alert('XSS')</script> [expect not 200] - SUCCESS");
                        ...
                        try {
                            ...
                            attachJaegerTrace("ts-admin-travel-service", "DELETE", "/api/v1/admintravelservice/admintravel/<script>alert('XSS')</script>", requestStartMicros, allStepParameters, false);
                        } catch (Exception e) {
                            Allure.parameter("🎯 Result", "✅ SUCCESS (response capture failed)");
                        }
                    } catch (Throwable t) {
                        stepResults.put(1, false);
                        System.out.println("❌ Step 1: ts-admin-travel-service DELETE /api/v1/admintravelservice/admintravel/<script>alert('XSS')</script> [expect not 200] - FAILED: " + t.getMessage());
                        ...
                        Allure.parameter("🔗 Failed Endpoint", "/api/v1/admintravelservice/admintravel/<script>alert('XSS')</script>");
                        ...
                        errorDetails.append("=====================================\n\n");
                        errorDetails.append("📋 Step: Step 1: ts-admin-travel-service DELETE /api/v1/admintravelservice/admintravel/<script>alert('XSS')</script> [expect not 200]\n");
                        errorDetails.append("🏢 Service: ts-admin-travel-service\n");
                        errorDetails.append("📡 Method: DELETE\n");
                        errorDetails.append("🔗 Endpoint: /api/v1/admintravelservice/admintravel/<script>alert('XSS')</script>\n");
                        ...
                        // ⏱️ Wait longer for trace propagation to Jaeger (increased delay)
                        try { Thread.sleep(3000); } catch (InterruptedException ie) { Thread.currentThread().interrupt(); }
                        attachJaegerTrace("ts-admin-travel-service", "DELETE", "/api/v1/admintravelservice/admintravel/<script>alert('XSS')</script>", requestStartMicros, allStepParameters, true);
                        
                        // 🔥 CRITICAL: Throw exception to mark step as FAILED (red arrow) in Allure
                        throw new RuntimeException("Step 1: ts-admin-travel-service DELETE /api/v1/admintravelservice/admintravel/<script>alert('XSS')</script> [expect not 200] failed: " + failureReason + " (" + errorType + ")", t);
                    }
                } else {
                    // ⏭️ SKIP: Clean skip reporting with proper Allure status
                    System.out.println("⏭️ SKIPPING: Step 1: ts-admin-travel-service DELETE /api/v1/admintravelservice/admintravel/<script>alert('XSS')</script> [expect not 200] - " + skipReason);
                    stepResults.put(1, false);
            ...
            if (stepException instanceof RuntimeException && stepException.getMessage().startsWith("Step failed:")) {
                ...
                System.out.println("⚠️ Step wrapper failed for Step 1: ts-admin-travel-service DELETE /api/v1/admintravelservice/admintravel/<script>alert('XSS')</script> [expect not 200]: " + stepException.getMessage());
                stepResults.put(1, false);
            }
        }
