...
public class get_api_v1_contactservice_contacts_account_1234567890_2201 {
    ...
    public void test_GET_2201_38() throws Exception {
        ...
        es.us.isa.restest.enhancer.TestResultCapture.setTestMetadata(
            "/api/v1/contactservice/contacts/account/1234567890", "GET", 
        ...
        // Step 1: ts-contacts-service GET /api/v1/contactservice/contacts/account/1234567890 [expect 200]
        ...
        try {
            Allure.step("Step 1: ts-contacts-service GET /api/v1/contactservice/contacts/account/1234567890 [expect 200]", () -> {
                Allure.parameter("🏢 Service", "ts-contacts-service");
                Allure.parameter("📡 HTTP Method", "GET");
                Allure.parameter("🔗 Endpoint", "/api/v1/contactservice/contacts/account/1234567890");
                Allure.parameter("🎯 Expected Status", 200);
                ...
                Allure.description("🎯 **Testing**: ts-contacts-service\n" +
                                 "📡 **Method**: GET\n" +
                                 "🔗 **Path**: /api/v1/contactservice/contacts/account/1234567890\n" +
                                 "🎯 **Expected**: 200\n" +
                ...
                if (!shouldSkip) {
                    System.out.println("▶️ EXECUTING: Step 1: ts-contacts-service GET /api/v1/contactservice/contacts/account/1234567890 [expect 200] (dependency analysis passed)");
                    ...
                    try {
                        ...
                        stepResponse1 = req.when().get("/api/v1/contactservice/contacts/account/1234567890")
                               .then().log().ifValidationFails()
                               .extract().response();
                            ...
                            try {
                                ...
                                if (softErrorCacheEnabled) {
                                    ...
                                    validationResult = llmValidator.validateResponseWithCache(
                                        ...
                                        "ts-contacts-service",
                                        "GET",
                                        "/api/v1/contactservice/contacts/account/1234567890",
                                        cache
                                    );
                                } else {
                        ...
                        System.out.println("✅ Step 1: ts-contacts-service GET /api/v1/contactservice/contacts/account/1234567890 [expect 200] - SUCCESS");
                        ...
                        try {
                            ...
                            attachJaegerTrace("ts-contacts-service", "GET", "/api/v1/contactservice/contacts/account/1234567890", requestStartMicros, allStepParameters, false);
                        } catch (Exception e) {
                            Allure.parameter("🎯 Result", "✅ SUCCESS (response capture failed)");
                        }
                    } catch (Throwable t) {
                        stepResults.put(1, false);
                        System.out.println("❌ Step 1: ts-contacts-service GET /api/v1/contactservice/contacts/account/1234567890 [expect 200] - FAILED: " + t.getMessage());
                        ...
                        Allure.parameter("🔗 Failed Endpoint", "/api/v1/contactservice/contacts/account/1234567890");
                        ...
                        errorDetails.append("=====================================\n\n");
                        errorDetails.append("📋 Step: Step 1: ts-contacts-service GET /api/v1/contactservice/contacts/account/1234567890 [expect 200]\n");
                        errorDetails.append("🏢 Service: ts-contacts-service\n");
                        errorDetails.append("📡 Method: GET\n");
                        errorDetails.append("🔗 Endpoint: /api/v1/contactservice/contacts/account/1234567890\n");
                        ...
                        // ⏱️ Wait longer for trace propagation to Jaeger (increased delay)
                        try { Thread.sleep(3000); } catch (InterruptedException ie) { Thread.currentThread().interrupt(); }
                        attachJaegerTrace("ts-contacts-service", "GET", "/api/v1/contactservice/contacts/account/1234567890", requestStartMicros, allStepParameters, true);
                        
                        // 🔥 CRITICAL: Throw exception to mark step as FAILED (red arrow) in Allure
                        throw new RuntimeException("Step 1: ts-contacts-service GET /api/v1/contactservice/contacts/account/1234567890 [expect 200] failed: " + failureReason + " (" + errorType + ")", t);
                    }
                } else {
                    // ⏭️ SKIP: Clean skip reporting with proper Allure status
                    System.out.println("⏭️ SKIPPING: Step 1: ts-contacts-service GET /api/v1/contactservice/contacts/account/1234567890 [expect 200] - " + skipReason);
                    stepResults.put(1, false);
            ...
            if (stepException instanceof RuntimeException && stepException.getMessage().startsWith("Step failed:")) {
                ...
                System.out.println("⚠️ Step wrapper failed for Step 1: ts-contacts-service GET /api/v1/contactservice/contacts/account/1234567890 [expect 200]: " + stepException.getMessage());
                stepResults.put(1, false);
            }
        }
    ...
    @Test
    public void test_GET_2201_77() throws Exception {
        // Record test execution for fault detection tracking
        es.us.isa.restest.analysis.FaultDetectionTracker.getInstance()
            .recordTestCase(this.getClass().getName(), "test_GET_2201_77");
                                ...
                                if (dataValue instanceof org.json.JSONObject) {
                                    ...
                                    if (dataObj.optBoolean("injected", false)) {
                                        ...
                                        if (!detectedFaultName.isEmpty()) {
                                            ...
                                            System.out.println("🔍 FAULT DETECTED: " + detectedFaultName + " in test: test_GET_2201_77");
                                        }
                                    }
                                }
                            }
                                    ...
                                    if (dataValue instanceof org.json.JSONObject) {
                                        ...
                                        if (dataObj.optBoolean("injected", false)) {
                                            ...
                                            if (!detectedFaultName.isEmpty()) {
                                                ...
                                                System.out.println("🔍 FAULT DETECTED (in failed test): " + detectedFaultName + " in test: test_GET_2201_77");
                                            }
                                        }
                                    }
                                }
        ...
        Allure.label("severity", severity);
        Allure.label("feature", "Microservice Workflow");
        Allure.label("story", "test_GET_2201_77");
        Allure.description("Microservice test scenario with " + totalSteps + " steps.");
        
        System.out.println("=== SCENARIO RESULT ===");
        System.out.println("Scenario: test_GET_2201_77");
        System.out.println("Total Steps: " + totalSteps);
        System.out.println("Successful: " + successfulSteps);
        System.out.println("Failed: " + failedSteps);
        System.out.println("Login Status: " + (loginSucceeded.get() ? "SUCCESS" : "FAILED"));
