...
/**
 * IMPROVED ALLURE REPORTING DEMO
 * 
 ...
 */
public class ImprovedAllureDemo {
    ...
    public void testScenarioWithImprovedReporting() throws Exception {
        ...
        // Step 2: Simulate a failing step to demonstrate enhanced failure reporting
        boolean shouldExecuteStep2 = true;
        if (shouldExecuteStep2) {
            String __uuid = java.util.UUID.randomUUID().toString();
            io.qameta.allure.model.StepResult __sr = new io.qameta.allure.model.StepResult()
                    .setName("Step 2: ts-nonexistent-service GET /api/v1/nonexistent")
                    .setDescription("Service: ts-nonexistent-service | Method: GET | Path: /api/v1/nonexistent | Expected Status: 200");
            Allure.getLifecycle().startStep(__uuid, __sr);
            
            // Add step metadata as parameters
            Allure.parameter("Service", "ts-nonexistent-service");
            Allure.parameter("HTTP Method", "GET");
            Allure.parameter("Endpoint", "/api/v1/nonexistent");
            Allure.parameter("Expected Status", 200);
            ...
            try {
                ...
                Response stepResponse2 = req.when().get("/api/v1/nonexistent")
                   .then().log().ifValidationFails()
                   .statusCode(200)
                   .extract().response();
                stepResults.put(2, true);
                System.out.println("✓ Step 2: ts-nonexistent-service GET /api/v1/nonexistent - SUCCESS");
                Allure.getLifecycle().updateStep(s -> s.setStatus(Status.PASSED));
            } catch (Throwable t) {
                stepResults.put(2, false);
                System.out.println("✗ Step 2: ts-nonexistent-service GET /api/v1/nonexistent - FAILED: " + t.getMessage());
                ...
                errorDetails.append("Error Message: ").append(t.getMessage()).append("\n\n");
                errorDetails.append("STEP DETAILS:\n");
                errorDetails.append("Service: ts-nonexistent-service\n");
                errorDetails.append("Method: GET\n");
                errorDetails.append("Path: /api/v1/nonexistent\n");
                errorDetails.append("Expected Status: 200\n");
                ...
                Allure.addAttachment("Detailed Error Report • Step 2: ts-nonexistent-service GET /api/v1/nonexistent", "text/plain", errorDetails.toString());
            ...
            stepResults.put(2, false);
            System.out.println("⚠ Step 2: ts-nonexistent-service GET /api/v1/nonexistent - SKIPPED (dependency failed)");
            Allure.getLifecycle().startStep(
                java.util.UUID.randomUUID().toString(),
                new io.qameta.allure.model.StepResult()
                     .setName("Step 2: ts-nonexistent-service GET /api/v1/nonexistent")
        ...
        Allure.label("story", "ImprovedAllureDemo");
        Allure.description("Microservice test scenario with " + totalSteps + " steps. " +
                           "Generated using two-stage LLM + semantic expansion approach.");
        
        System.out.println("=== SCENARIO RESULT ===");
        System.out.println("Scenario: ImprovedAllureDemo");
