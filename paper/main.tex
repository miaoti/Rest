...
\title{MIST: A Trace-Driven, LLM-Assisted Tool for Multi-Service\\REST API Test Generation\thanks{Tool Demonstration and Data Showcase Track, ICSME 2026.}}
...
\textbf{End-to-end flow.} The user invokes \texttt{java -jar restest.jar <core>.properties}. The core file points at an MST-only file (decoupling MIST configuration from RESTest's classic generators). The flow is: ingest spec $\rightarrow$ build registry $\rightarrow$ extract scenarios $\rightarrow$ shatter and deduplicate $\rightarrow$ generate variants $\rightarrow$ Sniper-mutate negatives $\rightarrow$ emit JUnit tests $\rightarrow$ execute in-process $\rightarrow$ trace-aware oracle $\rightarrow$ Allure + fault-detection report.
...
\textbf{REST API testing.} EvoMaster \cite{Arcuri2017EvoMaster,Arcuri2019EvoMasterTOSEM} pioneered evolutionary search-based test generation; RESTler \cite{Atlidakis2019RESTler} introduced stateful fuzzing; RESTest \cite{MartinLopez2021RESTest}, on which MIST is built, contributed the constraint-based and ART generators. Schemathesis \cite{HatfieldDodds2022Schemathesis} derives semantics-aware fuzzers from schemas; Morest \cite{Liu2022MoRest} and ARAT-RL \cite{Kim2023ARATRL} use feedback to guide generation; DeepREST \cite{Corradini2024DeepREST} applies deep RL. RESTGPT \cite{Kim2024RESTGPT}, LlamaRestTest \cite{Kim2025LlamaRestTest}, and AutoRestTest \cite{Stennett2025AutoRestTest} integrate LLMs for input synthesis. None of these tools centre cross-service trace-as-oracle reasoning; they target single-service APIs and use status-class oracles.
