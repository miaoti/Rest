package trainticket_twostage_test.TrainTicketTwoStageTest_1769819734301;
...
public class post_api_v1_travelplanservice_travelplan_minstation_1 {
            ...
            Allure.step("Step 1: ts-travel-plan-service POST /api/v1/travelplanservice/travelPlan/minStation [expect not 200]", () -> {
                ...
                if (!shouldSkip) {
                    ...
                    try {
                        ...
                        // 🔥 FIX: Set Content-Type to application/json for requests with bodies
                        req = req.contentType("application/json");
                        String requestBody1 = "{\"departureTime\":\"2026-02-15\",\"endPlace\":123,\"startPlace\":\"Shanghai\"}";
                        req = req.body(requestBody1);
                        ...
                        allStepParameters.put("body", "{\"departureTime\":\"2026-02-15\",\"endPlace\":123,\"startPlace\":\"Shanghai\"}");
                        // 🔥 FIX: Extract response FIRST (before status code assertion) to capture response body in all cases
                        stepResponse1 = req.when().post("/api/v1/travelplanservice/travelPlan/minStation")
                               .then().log().ifValidationFails()
                               .extract().response();
    ...
    @Test
    public void test_POST_1_1_explore_405_retry1() throws Exception {
        // Record test execution for fault detection tracking
        es.us.isa.restest.analysis.FaultDetectionTracker.getInstance()
            .recordTestCase(this.getClass().getName(), "test_POST_1_1_explore_405_retry1");
        ...
        Allure.label("parentSuite", "ts-travel-plan-service");
        Allure.label("suite", "POST /api/v1/travelplanservice/travelPlan/minStation");
        Allure.label("subSuite", "test_POST_1_1_explore_405_retry1");
            ...
            Allure.step("Step 1: ts-travel-plan-service POST /api/v1/travelplanservice/travelPlan/minStation [expect 405 (exploration)]", () -> {
                ...
                if (!shouldSkip) {
                    ...
                    try {
                        ...
                        // 🔥 FIX: Set Content-Type to application/json for requests with bodies
                        req = req.contentType("application/json");
                        String requestBody1 = "{\"departureTime\":\"2026-02-15\",\"startPlace\":\"Beijing\",\"endPlace\":\"Beijing\"}";
                        req = req.body(requestBody1);
                        ...
                        allStepParameters.put("body", "{\"departureTime\":\"2026-02-15\",\"startPlace\":\"Beijing\",\"endPlace\":\"Beijing\"}");
                        // 🔥 FIX: Extract response FIRST (before status code assertion) to capture response body in all cases
                        stepResponse1 = req.when().post("/api/v1/travelplanservice/travelPlan/minStation")
                               .then().log().ifValidationFails()
                               .extract().response();
                                    ...
                                    if (dataObj.optBoolean("injected", false)) {
                                        ...
                                        if (!detectedFaultName.isEmpty()) {
                                            es.us.isa.restest.analysis.FaultDetectionTracker.getInstance().recordDetectedFault(
                                                detectedFaultName,
                                                this.getClass().getName(),
                                                "test_POST_1_1_explore_405_retry1",
                                                System.currentTimeMillis(),
                                                faultCheckBody
                                            );
                                            System.out.println("🔍 FAULT DETECTED: " + detectedFaultName + " in test: test_POST_1_1_explore_405_retry1");
                                        }
                                    }
                                }
                            }
                                    ...
                                    if (dataValue instanceof org.json.JSONObject) {
                                        ...
                                        if (dataObj.optBoolean("injected", false)) {
                                            ...
                                            if (!detectedFaultName.isEmpty()) {
                                                ...
                                                System.out.println("🔍 FAULT DETECTED (in failed test): " + detectedFaultName + " in test: test_POST_1_1_explore_405_retry1");
                                            }
                                        }
                                    }
                                }
        ...
        Allure.label("severity", severity);
        Allure.label("feature", "Microservice Workflow");
        Allure.label("story", "test_POST_1_1_explore_405_retry1");
        Allure.description("Microservice test scenario with " + totalSteps + " steps.");
        
        System.out.println("=== SCENARIO RESULT ===");
        System.out.println("Scenario: test_POST_1_1_explore_405_retry1");
        System.out.println("Total Steps: " + totalSteps);
        System.out.println("Successful: " + successfulSteps);
        System.out.println("Failed: " + failedSteps);
        System.out.println("Login Status: " + (loginSucceeded.get() ? "SUCCESS" : "FAILED"));
