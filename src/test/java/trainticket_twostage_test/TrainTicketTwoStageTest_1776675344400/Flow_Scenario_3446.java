package trainticket_twostage_test.TrainTicketTwoStageTest_1776675344400;
...
@io.qameta.allure.junit4.DisplayName("Flow_Scenario_3446 | GET /api/v1/executeservice/execute/execute/{orderId}")
public class Flow_Scenario_3446 {
    ...
    @Test
    public void test_positive_flow_S3446_v1() throws Exception {
        // ═══════════════════════════════════════════════════════════════════════
        ...
        es.us.isa.restest.analysis.FaultDetectionTracker.getInstance()
            .recordTestCase(this.getClass().getName(), "test_positive_flow_S3446_v1");
                                    ...
                                    if (dataObj.optBoolean("injected", false)) {
                                        ...
                                        if (!detectedFaultName.isEmpty()) {
                                            es.us.isa.restest.analysis.FaultDetectionTracker.getInstance().recordDetectedFault(
                                                ...
                                                "test_positive_flow_S3446_v1",
                                                System.currentTimeMillis(),
                                                faultCheckBody
                                            );
                                            System.out.println("🔍 FAULT DETECTED: " + detectedFaultName + " in test: test_positive_flow_S3446_v1");
                                        }
                                    }
                                }
                            }
                                    ...
                                    if (dataValue instanceof org.json.JSONObject) {
                                        ...
                                        if (dataObj.optBoolean("injected", false)) {
                                            ...
                                            if (!detectedFaultName.isEmpty()) {
                                                ...
                                                System.out.println("🔍 FAULT DETECTED (in failed test): " + detectedFaultName + " in test: test_positive_flow_S3446_v1");
                                            }
                                        }
                                    }
                                }
        ...
        System.out.println("Scenario: test_positive_flow_S3446_v1");
    ...
    @Test
    public void test_positive_flow_S3446_v3() throws Exception {
        // ═══════════════════════════════════════════════════════════════════════
        // 🔧 ENHANCED TEST - Modified by Test Case Enhancer
        // Original Status: 200
        // Enhanced Parameters: [orderId]
        ...
        // Record test execution for fault detection tracking
        es.us.isa.restest.analysis.FaultDetectionTracker.getInstance()
            .recordTestCase(this.getClass().getName(), "test_positive_flow_S3446_v3");
        ...
        // Root 1: ts-execute-service GET /api/v1/executeservice/execute/execute/XYZ7890PQR [expect 200]
        es.us.isa.restest.enhancer.TestResultCapture.setStepMetadata(
            1, "/api/v1/executeservice/execute/execute/XYZ7890PQR", "GET", 
            "ts-execute-service", false);
        es.us.isa.restest.enhancer.TestResultCapture.addParameter(
            "orderId", "XYZ7890PQR", 
            "string", "path", null, null, true, 1, false);
        ...
        final long requestStartMicros = System.currentTimeMillis() * 1000L;
        try {
            Allure.step("Root 1: ts-execute-service GET /api/v1/executeservice/execute/execute/XYZ7890PQR [expect 200]", () -> {
                Allure.parameter("🏢 Service", "ts-execute-service");
                Allure.parameter("📡 HTTP Method", "GET");
                Allure.parameter("🔗 Endpoint", "/api/v1/executeservice/execute/execute/XYZ7890PQR");
                Allure.parameter("🎯 Expected Status", 200);
                Allure.parameter("🔗 Dependency Type", "INDEPENDENT (can execute regardless of other step results)");
                Allure.parameter("📌 Step ID", "R1");
                Allure.description("🎯 **Testing**: ts-execute-service\n" +
                                 "📡 **Method**: GET\n" +
                                 "🔗 **Path**: /api/v1/executeservice/execute/execute/XYZ7890PQR\n" +
                                 "🎯 **Expected**: 200\n" +
                                 "🔗 **Dependencies**: INDEPENDENT (can execute regardless of other step results)");
                ...
                if (!shouldSkip) {
                    System.out.println("▶️ EXECUTING: Root 1: ts-execute-service GET /api/v1/executeservice/execute/execute/XYZ7890PQR [expect 200] (dependency analysis passed)");
                    Response stepResponse1 = null;
                    ...
                    try {
                        ...
                        // 🔥 FIX: Extract response FIRST (before status code assertion) to capture response body in all cases
                        stepResponse1 = req.when().get("/api/v1/executeservice/execute/execute/XYZ7890PQR")
                               .then().log().ifValidationFails()
                               .extract().response();
                            ...
                            try {
                                ...
                                if (SOFT_ERROR_CACHE_ENABLED && ruleCache != null) {
                                    validationResult = llmValidator.validateResponseWithCache(
                                        ...
                                        "ts-execute-service",
                                        "GET",
                                        "/api/v1/executeservice/execute/execute/XYZ7890PQR",
                                        ruleCache
                                    );
                                } else {
                                    validationResult = llmValidator.validateResponse(
                                    ...
                                    if (dataObj.optBoolean("injected", false)) {
                                        ...
                                        if (!detectedFaultName.isEmpty()) {
                                            es.us.isa.restest.analysis.FaultDetectionTracker.getInstance().recordDetectedFault(
                                                detectedFaultName,
                                                this.getClass().getName(),
                                                "test_positive_flow_S3446_v3",
                                                System.currentTimeMillis(),
                                                faultCheckBody
                                            );
                                            System.out.println("🔍 FAULT DETECTED: " + detectedFaultName + " in test: test_positive_flow_S3446_v3");
                                        }
                                    }
                                }
                            }
                        ...
                        stepResults.put(1, true);
                        System.out.println("✅ Root 1: ts-execute-service GET /api/v1/executeservice/execute/execute/XYZ7890PQR [expect 200] - SUCCESS");
                        try {
                            String fullBody1 = stepResponse1.getBody().asString();
                            if (fullBody1 != null && !fullBody1.isEmpty()) {
                                capturedOutputs.put(1, fullBody1);
                        ...
                        try {
                            ...
                            // ⏱️ Wait longer for trace propagation to Jaeger (increased delay)
                            try { Thread.sleep(3000); } catch (InterruptedException ie) { Thread.currentThread().interrupt(); }
                            attachJaegerTrace("ts-execute-service", "GET", "/api/v1/executeservice/execute/execute/XYZ7890PQR", requestStartMicros, allStepParameters, false);
                        } catch (Exception e) {
                            Allure.parameter("🎯 Result", "✅ SUCCESS (response capture failed)");
                        }
                    } catch (Throwable t) {
                        stepResults.put(1, false);
                        System.out.println("❌ Root 1: ts-execute-service GET /api/v1/executeservice/execute/execute/XYZ7890PQR [expect 200] - FAILED: " + t.getMessage());
                                    ...
                                    if (dataValue instanceof org.json.JSONObject) {
                                        ...
                                        if (dataObj.optBoolean("injected", false)) {
                                            ...
                                            if (!detectedFaultName.isEmpty()) {
                                                ...
                                                System.out.println("🔍 FAULT DETECTED (in failed test): " + detectedFaultName + " in test: test_positive_flow_S3446_v3");
                                            }
                                        }
                                    }
                                }
                        ...
                        Allure.parameter("🏢 Failed Service", "ts-execute-service");
                        Allure.parameter("📡 Failed Method", "GET");
                        Allure.parameter("🔗 Failed Endpoint", "/api/v1/executeservice/execute/execute/XYZ7890PQR");
                        ...
                        errorDetails.append("❌ STEP FAILURE ANALYSIS\n");
                        errorDetails.append("=====================================\n\n");
                        errorDetails.append("📋 Step: Root 1: ts-execute-service GET /api/v1/executeservice/execute/execute/XYZ7890PQR [expect 200]\n");
                        errorDetails.append("🏢 Service: ts-execute-service\n");
                        errorDetails.append("📡 Method: GET\n");
                        errorDetails.append("🔗 Endpoint: /api/v1/executeservice/execute/execute/XYZ7890PQR\n");
                        errorDetails.append("💥 Error Type: ").append(errorType).append("\n");
                        errorDetails.append("🔍 Reason: ").append(failureReason).append("\n\n");
                        errorDetails.append("📜 Full Error Message:\n");
                        errorDetails.append(t.getMessage() != null ? t.getMessage() : "No message").append("\n\n");
                        ...
                        // ⏱️ Wait longer for trace propagation to Jaeger (increased delay)
                        try { Thread.sleep(3000); } catch (InterruptedException ie) { Thread.currentThread().interrupt(); }
                        attachJaegerTrace("ts-execute-service", "GET", "/api/v1/executeservice/execute/execute/XYZ7890PQR", requestStartMicros, allStepParameters, true);
                        
                        // 🔥 CRITICAL: Throw exception to mark step as FAILED (red arrow) in Allure
                        throw new RuntimeException("Root 1: ts-execute-service GET /api/v1/executeservice/execute/execute/XYZ7890PQR [expect 200] failed: " + failureReason + " (" + errorType + ")", t);
                    }
                } else {
                    // ⏭️ SKIP: Clean skip reporting with proper Allure status
                    System.out.println("⏭️ SKIPPING: Root 1: ts-execute-service GET /api/v1/executeservice/execute/execute/XYZ7890PQR [expect 200] - " + skipReason);
                    stepResults.put(1, false);
            ...
            if (stepException instanceof RuntimeException && stepException.getMessage().startsWith("Step failed:")) {
                ...
                // Unexpected wrapper failure
                System.out.println("⚠️ Step wrapper failed for Root 1: ts-execute-service GET /api/v1/executeservice/execute/execute/XYZ7890PQR [expect 200]: " + stepException.getMessage());
                stepResults.put(1, false);
            }
        }
        } // end per-step scope block
        ...
        System.out.println("=== SCENARIO RESULT ===");
        System.out.println("Scenario: test_positive_flow_S3446_v3");
        System.out.println("Total Steps: " + totalSteps);
        System.out.println("Successful: " + successfulSteps);
        System.out.println("Failed: " + failedSteps);
        System.out.println("Login Status: " + (loginSucceeded.get() ? "SUCCESS" : "FAILED"));
    ...
    @Test
    public void test_positive_flow_S3446_v4() throws Exception {
        // ═══════════════════════════════════════════════════════════════════════
        // 🔧 ENHANCED TEST - Modified by Test Case Enhancer
        // Original Status: 200
        // Enhanced Parameters: [orderId]
        ...
        // Record test execution for fault detection tracking
        es.us.isa.restest.analysis.FaultDetectionTracker.getInstance()
            .recordTestCase(this.getClass().getName(), "test_positive_flow_S3446_v4");
        ...
        // Root 1: ts-execute-service GET /api/v1/executeservice/execute/execute/QWEASDZXC9 [expect 200]
        es.us.isa.restest.enhancer.TestResultCapture.setStepMetadata(
            1, "/api/v1/executeservice/execute/execute/QWEASDZXC9", "GET", 
            "ts-execute-service", false);
        es.us.isa.restest.enhancer.TestResultCapture.addParameter(
            "orderId", "QWEASDZXC9", 
            "string", "path", null, null, true, 1, false);
        ...
        final long requestStartMicros = System.currentTimeMillis() * 1000L;
        try {
            Allure.step("Root 1: ts-execute-service GET /api/v1/executeservice/execute/execute/QWEASDZXC9 [expect 200]", () -> {
                Allure.parameter("🏢 Service", "ts-execute-service");
                Allure.parameter("📡 HTTP Method", "GET");
                Allure.parameter("🔗 Endpoint", "/api/v1/executeservice/execute/execute/QWEASDZXC9");
                Allure.parameter("🎯 Expected Status", 200);
                Allure.parameter("🔗 Dependency Type", "INDEPENDENT (can execute regardless of other step results)");
                Allure.parameter("📌 Step ID", "R1");
                Allure.description("🎯 **Testing**: ts-execute-service\n" +
                                 "📡 **Method**: GET\n" +
                                 "🔗 **Path**: /api/v1/executeservice/execute/execute/QWEASDZXC9\n" +
                                 "🎯 **Expected**: 200\n" +
                                 "🔗 **Dependencies**: INDEPENDENT (can execute regardless of other step results)");
                ...
                if (!shouldSkip) {
                    System.out.println("▶️ EXECUTING: Root 1: ts-execute-service GET /api/v1/executeservice/execute/execute/QWEASDZXC9 [expect 200] (dependency analysis passed)");
                    Response stepResponse1 = null;
                    ...
                    try {
                        ...
                        // 🔥 FIX: Extract response FIRST (before status code assertion) to capture response body in all cases
                        stepResponse1 = req.when().get("/api/v1/executeservice/execute/execute/QWEASDZXC9")
                               .then().log().ifValidationFails()
                               .extract().response();
                            ...
                            try {
                                ...
                                if (SOFT_ERROR_CACHE_ENABLED && ruleCache != null) {
                                    validationResult = llmValidator.validateResponseWithCache(
                                        ...
                                        "ts-execute-service",
                                        "GET",
                                        "/api/v1/executeservice/execute/execute/QWEASDZXC9",
                                        ruleCache
                                    );
                                } else {
                                    validationResult = llmValidator.validateResponse(
                                    ...
                                    if (dataObj.optBoolean("injected", false)) {
                                        ...
                                        if (!detectedFaultName.isEmpty()) {
                                            es.us.isa.restest.analysis.FaultDetectionTracker.getInstance().recordDetectedFault(
                                                detectedFaultName,
                                                this.getClass().getName(),
                                                "test_positive_flow_S3446_v4",
                                                System.currentTimeMillis(),
                                                faultCheckBody
                                            );
                                            System.out.println("🔍 FAULT DETECTED: " + detectedFaultName + " in test: test_positive_flow_S3446_v4");
                                        }
                                    }
                                }
                            }
                        ...
                        stepResults.put(1, true);
                        System.out.println("✅ Root 1: ts-execute-service GET /api/v1/executeservice/execute/execute/QWEASDZXC9 [expect 200] - SUCCESS");
                        try {
                            String fullBody1 = stepResponse1.getBody().asString();
                            if (fullBody1 != null && !fullBody1.isEmpty()) {
                                capturedOutputs.put(1, fullBody1);
                        ...
                        try {
                            ...
                            // ⏱️ Wait longer for trace propagation to Jaeger (increased delay)
                            try { Thread.sleep(3000); } catch (InterruptedException ie) { Thread.currentThread().interrupt(); }
                            attachJaegerTrace("ts-execute-service", "GET", "/api/v1/executeservice/execute/execute/QWEASDZXC9", requestStartMicros, allStepParameters, false);
                        } catch (Exception e) {
                            Allure.parameter("🎯 Result", "✅ SUCCESS (response capture failed)");
                        }
                    } catch (Throwable t) {
                        stepResults.put(1, false);
                        System.out.println("❌ Root 1: ts-execute-service GET /api/v1/executeservice/execute/execute/QWEASDZXC9 [expect 200] - FAILED: " + t.getMessage());
                                    ...
                                    if (dataValue instanceof org.json.JSONObject) {
                                        ...
                                        if (dataObj.optBoolean("injected", false)) {
                                            ...
                                            if (!detectedFaultName.isEmpty()) {
                                                ...
                                                System.out.println("🔍 FAULT DETECTED (in failed test): " + detectedFaultName + " in test: test_positive_flow_S3446_v4");
                                            }
                                        }
                                    }
                                }
                        ...
                        Allure.parameter("🏢 Failed Service", "ts-execute-service");
                        Allure.parameter("📡 Failed Method", "GET");
                        Allure.parameter("🔗 Failed Endpoint", "/api/v1/executeservice/execute/execute/QWEASDZXC9");
                        ...
                        errorDetails.append("❌ STEP FAILURE ANALYSIS\n");
                        errorDetails.append("=====================================\n\n");
                        errorDetails.append("📋 Step: Root 1: ts-execute-service GET /api/v1/executeservice/execute/execute/QWEASDZXC9 [expect 200]\n");
                        errorDetails.append("🏢 Service: ts-execute-service\n");
                        errorDetails.append("📡 Method: GET\n");
                        errorDetails.append("🔗 Endpoint: /api/v1/executeservice/execute/execute/QWEASDZXC9\n");
                        errorDetails.append("💥 Error Type: ").append(errorType).append("\n");
                        errorDetails.append("🔍 Reason: ").append(failureReason).append("\n\n");
                        errorDetails.append("📜 Full Error Message:\n");
                        errorDetails.append(t.getMessage() != null ? t.getMessage() : "No message").append("\n\n");
                        ...
                        // ⏱️ Wait longer for trace propagation to Jaeger (increased delay)
                        try { Thread.sleep(3000); } catch (InterruptedException ie) { Thread.currentThread().interrupt(); }
                        attachJaegerTrace("ts-execute-service", "GET", "/api/v1/executeservice/execute/execute/QWEASDZXC9", requestStartMicros, allStepParameters, true);
                        
                        // 🔥 CRITICAL: Throw exception to mark step as FAILED (red arrow) in Allure
                        throw new RuntimeException("Root 1: ts-execute-service GET /api/v1/executeservice/execute/execute/QWEASDZXC9 [expect 200] failed: " + failureReason + " (" + errorType + ")", t);
                    }
                } else {
                    // ⏭️ SKIP: Clean skip reporting with proper Allure status
                    System.out.println("⏭️ SKIPPING: Root 1: ts-execute-service GET /api/v1/executeservice/execute/execute/QWEASDZXC9 [expect 200] - " + skipReason);
                    stepResults.put(1, false);
            ...
            if (stepException instanceof RuntimeException && stepException.getMessage().startsWith("Step failed:")) {
                ...
                // Unexpected wrapper failure
                System.out.println("⚠️ Step wrapper failed for Root 1: ts-execute-service GET /api/v1/executeservice/execute/execute/QWEASDZXC9 [expect 200]: " + stepException.getMessage());
                stepResults.put(1, false);
            }
        }
        } // end per-step scope block
        ...
        System.out.println("=== SCENARIO RESULT ===");
        System.out.println("Scenario: test_positive_flow_S3446_v4");
        System.out.println("Total Steps: " + totalSteps);
        System.out.println("Successful: " + successfulSteps);
        System.out.println("Failed: " + failedSteps);
        System.out.println("Login Status: " + (loginSucceeded.get() ? "SUCCESS" : "FAILED"));
    ...
    @Test
    public void test_positive_flow_S3446_v5() throws Exception {
        // ═══════════════════════════════════════════════════════════════════════
        // 🔧 ENHANCED TEST - Modified by Test Case Enhancer
        // Original Status: 200
        // Enhanced Parameters: [orderId]
        ...
        // Record test execution for fault detection tracking
        es.us.isa.restest.analysis.FaultDetectionTracker.getInstance()
            .recordTestCase(this.getClass().getName(), "test_positive_flow_S3446_v5");
                                    ...
                                    if (dataObj.optBoolean("injected", false)) {
                                        ...
                                        if (!detectedFaultName.isEmpty()) {
                                            es.us.isa.restest.analysis.FaultDetectionTracker.getInstance().recordDetectedFault(
                                                detectedFaultName,
                                                this.getClass().getName(),
                                                "test_positive_flow_S3446_v5",
                                                System.currentTimeMillis(),
                                                faultCheckBody
                                            );
                                            System.out.println("🔍 FAULT DETECTED: " + detectedFaultName + " in test: test_positive_flow_S3446_v5");
                                        }
                                    }
                                }
                            }
                                    ...
                                    if (dataValue instanceof org.json.JSONObject) {
                                        ...
                                        if (dataObj.optBoolean("injected", false)) {
                                            ...
                                            if (!detectedFaultName.isEmpty()) {
                                                ...
                                                System.out.println("🔍 FAULT DETECTED (in failed test): " + detectedFaultName + " in test: test_positive_flow_S3446_v5");
                                            }
                                        }
                                    }
                                }
        ...
        System.out.println("=== SCENARIO RESULT ===");
        System.out.println("Scenario: test_positive_flow_S3446_v5");
        System.out.println("Total Steps: " + totalSteps);
        System.out.println("Successful: " + successfulSteps);
        System.out.println("Failed: " + failedSteps);
        System.out.println("Login Status: " + (loginSucceeded.get() ? "SUCCESS" : "FAILED"));
    ...
    @Test
    public void test_positive_flow_S3446_v6() throws Exception {
        // ═══════════════════════════════════════════════════════════════════════
        // 🔧 ENHANCED TEST - Modified by Test Case Enhancer
        // Original Status: 200
        // Enhanced Parameters: [orderId]
        // ═══════════════════════════════════════════════════════════════════════
        Allure.label("enhancement", "ENHANCED");
        Allure.addAttachment("📝 Original Failure", "text/plain", "Original Status: 200\nOriginal Response: {\"status\":0,\"msg\":\"Order Not Found\",\"data\":null}\nEnhanced Parameters: {orderId=XYZ789012}");

        // Record test execution for fault detection tracking
        es.us.isa.restest.analysis.FaultDetectionTracker.getInstance()
            .recordTestCase(this.getClass().getName(), "test_positive_flow_S3446_v6");
        ...
        // Root 1: ts-execute-service GET /api/v1/executeservice/execute/execute/ABCD123456 [expect 200]
        es.us.isa.restest.enhancer.TestResultCapture.setStepMetadata(
            1, "/api/v1/executeservice/execute/execute/ABCD123456", "GET", 
            "ts-execute-service", false);
        es.us.isa.restest.enhancer.TestResultCapture.addParameter(
            "orderId", "ABCD123456", 
            "string", "path", null, null, true, 1, false);
        ...
        final long requestStartMicros = System.currentTimeMillis() * 1000L;
        try {
            Allure.step("Root 1: ts-execute-service GET /api/v1/executeservice/execute/execute/ABCD123456 [expect 200]", () -> {
                Allure.parameter("🏢 Service", "ts-execute-service");
                Allure.parameter("📡 HTTP Method", "GET");
                Allure.parameter("🔗 Endpoint", "/api/v1/executeservice/execute/execute/ABCD123456");
                Allure.parameter("🎯 Expected Status", 200);
                Allure.parameter("🔗 Dependency Type", "INDEPENDENT (can execute regardless of other step results)");
                Allure.parameter("📌 Step ID", "R1");
                Allure.description("🎯 **Testing**: ts-execute-service\n" +
                                 "📡 **Method**: GET\n" +
                                 "🔗 **Path**: /api/v1/executeservice/execute/execute/ABCD123456\n" +
                                 "🎯 **Expected**: 200\n" +
                                 "🔗 **Dependencies**: INDEPENDENT (can execute regardless of other step results)");
                ...
                if (!shouldSkip) {
                    System.out.println("▶️ EXECUTING: Root 1: ts-execute-service GET /api/v1/executeservice/execute/execute/ABCD123456 [expect 200] (dependency analysis passed)");
                    Response stepResponse1 = null;
                    ...
                    try {
                        ...
                        // 🔥 FIX: Extract response FIRST (before status code assertion) to capture response body in all cases
                        stepResponse1 = req.when().get("/api/v1/executeservice/execute/execute/ABCD123456")
                               .then().log().ifValidationFails()
                               .extract().response();
                            ...
                            try {
                                ...
                                if (SOFT_ERROR_CACHE_ENABLED && ruleCache != null) {
                                    validationResult = llmValidator.validateResponseWithCache(
                                        ...
                                        "ts-execute-service",
                                        "GET",
                                        "/api/v1/executeservice/execute/execute/ABCD123456",
                                        ruleCache
                                    );
                                } else {
                                    validationResult = llmValidator.validateResponse(
                                    ...
                                    if (dataObj.optBoolean("injected", false)) {
                                        ...
                                        if (!detectedFaultName.isEmpty()) {
                                            es.us.isa.restest.analysis.FaultDetectionTracker.getInstance().recordDetectedFault(
                                                detectedFaultName,
                                                this.getClass().getName(),
                                                "test_positive_flow_S3446_v6",
                                                System.currentTimeMillis(),
                                                faultCheckBody
                                            );
                                            System.out.println("🔍 FAULT DETECTED: " + detectedFaultName + " in test: test_positive_flow_S3446_v6");
                                        }
                                    }
                                }
                            }
                        ...
                        stepResults.put(1, true);
                        System.out.println("✅ Root 1: ts-execute-service GET /api/v1/executeservice/execute/execute/ABCD123456 [expect 200] - SUCCESS");
                        try {
                            String fullBody1 = stepResponse1.getBody().asString();
                            if (fullBody1 != null && !fullBody1.isEmpty()) {
                                capturedOutputs.put(1, fullBody1);
                        ...
                        try {
                            ...
                            // ⏱️ Wait longer for trace propagation to Jaeger (increased delay)
                            try { Thread.sleep(3000); } catch (InterruptedException ie) { Thread.currentThread().interrupt(); }
                            attachJaegerTrace("ts-execute-service", "GET", "/api/v1/executeservice/execute/execute/ABCD123456", requestStartMicros, allStepParameters, false);
                        } catch (Exception e) {
                            Allure.parameter("🎯 Result", "✅ SUCCESS (response capture failed)");
                        }
                    } catch (Throwable t) {
                        stepResults.put(1, false);
                        System.out.println("❌ Root 1: ts-execute-service GET /api/v1/executeservice/execute/execute/ABCD123456 [expect 200] - FAILED: " + t.getMessage());
                                    ...
                                    if (dataValue instanceof org.json.JSONObject) {
                                        ...
                                        if (dataObj.optBoolean("injected", false)) {
                                            ...
                                            if (!detectedFaultName.isEmpty()) {
                                                ...
                                                System.out.println("🔍 FAULT DETECTED (in failed test): " + detectedFaultName + " in test: test_positive_flow_S3446_v6");
                                            }
                                        }
                                    }
                                }
                        ...
                        Allure.parameter("🏢 Failed Service", "ts-execute-service");
                        Allure.parameter("📡 Failed Method", "GET");
                        Allure.parameter("🔗 Failed Endpoint", "/api/v1/executeservice/execute/execute/ABCD123456");
                        ...
                        errorDetails.append("❌ STEP FAILURE ANALYSIS\n");
                        errorDetails.append("=====================================\n\n");
                        errorDetails.append("📋 Step: Root 1: ts-execute-service GET /api/v1/executeservice/execute/execute/ABCD123456 [expect 200]\n");
                        errorDetails.append("🏢 Service: ts-execute-service\n");
                        errorDetails.append("📡 Method: GET\n");
                        errorDetails.append("🔗 Endpoint: /api/v1/executeservice/execute/execute/ABCD123456\n");
                        errorDetails.append("💥 Error Type: ").append(errorType).append("\n");
                        errorDetails.append("🔍 Reason: ").append(failureReason).append("\n\n");
                        errorDetails.append("📜 Full Error Message:\n");
                        errorDetails.append(t.getMessage() != null ? t.getMessage() : "No message").append("\n\n");
                        ...
                        // ⏱️ Wait longer for trace propagation to Jaeger (increased delay)
                        try { Thread.sleep(3000); } catch (InterruptedException ie) { Thread.currentThread().interrupt(); }
                        attachJaegerTrace("ts-execute-service", "GET", "/api/v1/executeservice/execute/execute/ABCD123456", requestStartMicros, allStepParameters, true);
                        
                        // 🔥 CRITICAL: Throw exception to mark step as FAILED (red arrow) in Allure
                        throw new RuntimeException("Root 1: ts-execute-service GET /api/v1/executeservice/execute/execute/ABCD123456 [expect 200] failed: " + failureReason + " (" + errorType + ")", t);
                    }
                } else {
                    // ⏭️ SKIP: Clean skip reporting with proper Allure status
                    System.out.println("⏭️ SKIPPING: Root 1: ts-execute-service GET /api/v1/executeservice/execute/execute/ABCD123456 [expect 200] - " + skipReason);
                    stepResults.put(1, false);
            ...
            if (stepException instanceof RuntimeException && stepException.getMessage().startsWith("Step failed:")) {
                ...
                // Unexpected wrapper failure
                System.out.println("⚠️ Step wrapper failed for Root 1: ts-execute-service GET /api/v1/executeservice/execute/execute/ABCD123456 [expect 200]: " + stepException.getMessage());
                stepResults.put(1, false);
            }
        }
        } // end per-step scope block
        ...
        System.out.println("=== SCENARIO RESULT ===");
        System.out.println("Scenario: test_positive_flow_S3446_v6");
        System.out.println("Total Steps: " + totalSteps);
        System.out.println("Successful: " + successfulSteps);
        System.out.println("Failed: " + failedSteps);
        System.out.println("Login Status: " + (loginSucceeded.get() ? "SUCCESS" : "FAILED"));
