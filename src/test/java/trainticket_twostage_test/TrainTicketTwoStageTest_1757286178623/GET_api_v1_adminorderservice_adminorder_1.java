package trainticket_twostage_test.TrainTicketTwoStageTest_1757286178623;
...
public class GET_api_v1_adminorderservice_adminorder_1 {
    ...
    private static String generateTraceTable(JSONObject trace) {
        try {
            ...
            table.append("═════════════════════════════════════════════════════════════════\n");
            table.append("              🔗 API CALL TRACE HIERARCHY              \n");
            table.append("═════════════════════════════════════════════════════════════════\n");
            if (!traceId.isEmpty()) {
            ...
            if (rootApiCalls.isEmpty()) {
                ...
                table.append("API HIERARCHY (Parent → Child)\n");
                table.append("─────────────────────────────────────────────────────────────────\n");
    ...
    private static void renderApiHierarchy(StringBuilder table, String spanId, Map<String, JSONObject> apiSpanById, Map<String, List<String>> apiChildren, JSONObject processes, int depth, AtomicInteger counter, String prefix) {
        ...
        // Smart truncation for narrow windows
        String displayService = serviceName.length() > 15 ? serviceName.substring(0, 12) + "..." : serviceName;
        String displayEndpoint = endpoint;
        
        // Extract just the path for readability
        if (endpoint.startsWith("http")) {
            try {
                int pathStart = endpoint.indexOf("/", 8);  // Skip http:// or https://
                if (pathStart > 0) {
        ...
        if (displayEndpoint.length() > 35) {
            displayEndpoint = displayEndpoint.substring(0, 32) + "...";
        }
        ...
        // Build compact format: [#] Service Method Path Status(time)
        line.append(String.format("[%d] %s %s %s %s(%s)", 
            counter.getAndIncrement(), displayService, method, displayEndpoint, statusIcon, durationStr));
