...
@Epic("Baseline Tests")
@Feature("ts-user-service")
@io.qameta.allure.junit4.DisplayName("Flow_Scenario_538 | GET /api/v1/userservice/users")
public class Flow_Scenario_538 {
    ...
    @Test
    public void test_positive_flow_S538_v1() throws Exception {
        // Record test execution for fault detection tracking
        es.us.isa.restest.analysis.FaultDetectionTracker.getInstance()
            .recordTestCase(this.getClass().getName(), "test_positive_flow_S538_v1");
        ...
        Allure.label("parentSuite", "ts-user-service");
        ...
        // Root 1: ts-user-service GET /api/v1/userservice/users [expect 200]
        es.us.isa.restest.enhancer.TestResultCapture.setStepMetadata(
            1, "/api/v1/userservice/users", "GET", 
            "ts-user-service", false);
        ...
        try {
            Allure.step("Root 1: ts-user-service GET /api/v1/userservice/users [expect 200]", () -> {
                ...
                if (!shouldSkip) {
                    System.out.println("▶️ EXECUTING: Root 1: ts-user-service GET /api/v1/userservice/users [expect 200] (dependency analysis passed)");
                    Response stepResponse1 = null;
                    
                    try {
                        RequestSpecification req = RestAssured.given();
                        req = es.us.isa.restest.auth.MstAuthHandler.applyAuth(req, "/api/v1/userservice/users", __mstOverrideToken, __mstDisableAuth);
                                    ...
                                    if (dataObj.optBoolean("injected", false)) {
                                        ...
                                        if (!detectedFaultName.isEmpty()) {
                                            es.us.isa.restest.analysis.FaultDetectionTracker.getInstance().recordDetectedFault(
                                                ...
                                                "test_positive_flow_S538_v1",
                                                System.currentTimeMillis(),
                                                faultCheckBody
                                            );
                                            System.out.println("🔍 FAULT DETECTED: " + detectedFaultName + " in test: test_positive_flow_S538_v1");
                                        }
                                    }
                                }
                            }
                        ...
                        System.out.println("✅ Root 1: ts-user-service GET /api/v1/userservice/users [expect 200] - SUCCESS");
                        try {
                        ...
                        System.out.println("❌ Root 1: ts-user-service GET /api/v1/userservice/users [expect 200] - FAILED: " + t.getMessage());
                                    ...
                                    if (dataValue instanceof org.json.JSONObject) {
                                        ...
                                        if (dataObj.optBoolean("injected", false)) {
                                            ...
                                            if (!detectedFaultName.isEmpty()) {
                                                ...
                                                System.out.println("🔍 FAULT DETECTED (in failed test): " + detectedFaultName + " in test: test_positive_flow_S538_v1");
                                            }
                                        }
                                    }
                                }
                        ...
                        errorDetails.append("=====================================\n\n");
                        errorDetails.append("📋 Step: Root 1: ts-user-service GET /api/v1/userservice/users [expect 200]\n");
                        ...
                        throw new RuntimeException("Root 1: ts-user-service GET /api/v1/userservice/users [expect 200] failed: " + failureReason + " (" + errorType + ")", t);
                    }
                } else {
                    // ⏭️ SKIP: Clean skip reporting with proper Allure status
                    System.out.println("⏭️ SKIPPING: Root 1: ts-user-service GET /api/v1/userservice/users [expect 200] - " + skipReason);
                    stepResults.put(1, false);
            ...
            if (stepException instanceof RuntimeException && stepException.getMessage().startsWith("Step failed:")) {
                ...
                System.out.println("⚠️ Step wrapper failed for Root 1: ts-user-service GET /api/v1/userservice/users [expect 200]: " + stepException.getMessage());
                stepResults.put(1, false);
            }
        }
        ...
        System.out.println("Scenario: test_positive_flow_S538_v1");
