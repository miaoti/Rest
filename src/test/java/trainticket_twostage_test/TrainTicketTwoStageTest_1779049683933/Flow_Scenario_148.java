...
@Epic("Baseline Tests")
@Feature("ts-route-service")
@io.qameta.allure.junit4.DisplayName("Flow_Scenario_148 | GET /api/v1/routeservice/routes")
public class Flow_Scenario_148 {
    ...
    public void test_positive_flow_S148_v1() throws Exception {
        ...
        Allure.label("parentSuite", "ts-route-service");
        ...
        // Root 1: ts-route-service GET /api/v1/routeservice/routes [expect 200]
        es.us.isa.restest.enhancer.TestResultCapture.setStepMetadata(
            1, "/api/v1/routeservice/routes", "GET", 
            "ts-route-service", false);
        ...
        try {
            Allure.step("Root 1: ts-route-service GET /api/v1/routeservice/routes [expect 200]", () -> {
                ...
                if (!shouldSkip) {
                    System.out.println("▶️ EXECUTING: Root 1: ts-route-service GET /api/v1/routeservice/routes [expect 200] (dependency analysis passed)");
                    Response stepResponse1 = null;
                    
                    try {
                        RequestSpecification req = RestAssured.given();
                        req = es.us.isa.restest.auth.MstAuthHandler.applyAuth(req, "/api/v1/routeservice/routes", __mstOverrideToken, __mstDisableAuth);
                        ...
                        stepResponse1 = req.when().get("/api/v1/routeservice/routes")
                        ...
                        System.out.println("✅ Root 1: ts-route-service GET /api/v1/routeservice/routes [expect 200] - SUCCESS");
                        try {
                        ...
                        try {
                            ...
                            attachJaegerTrace("ts-route-service", "GET", "/api/v1/routeservice/routes", requestStartMicros, allStepParameters, false);
                        } catch (Exception e) {
                            Allure.parameter("🎯 Result", "✅ SUCCESS (response capture failed)");
                        }
                    } catch (Throwable t) {
                        stepResults.put(1, false);
                        System.out.println("❌ Root 1: ts-route-service GET /api/v1/routeservice/routes [expect 200] - FAILED: " + t.getMessage());
                        ...
                        Allure.parameter("🏢 Failed Service", "ts-route-service");
                        Allure.parameter("📡 Failed Method", "GET");
                        Allure.parameter("🔗 Failed Endpoint", "/api/v1/routeservice/routes");
                        ...
                        errorDetails.append("=====================================\n\n");
                        errorDetails.append("📋 Step: Root 1: ts-route-service GET /api/v1/routeservice/routes [expect 200]\n");
                        errorDetails.append("🏢 Service: ts-route-service\n");
                        errorDetails.append("📡 Method: GET\n");
                        errorDetails.append("🔗 Endpoint: /api/v1/routeservice/routes\n");
                        ...
                        // ⏱️ Wait longer for trace propagation to Jaeger (increased delay)
                        try { Thread.sleep(3000); } catch (InterruptedException ie) { Thread.currentThread().interrupt(); }
                        attachJaegerTrace("ts-route-service", "GET", "/api/v1/routeservice/routes", requestStartMicros, allStepParameters, true);
                        
                        // 🔥 CRITICAL: Throw exception to mark step as FAILED (red arrow) in Allure
                        throw new RuntimeException("Root 1: ts-route-service GET /api/v1/routeservice/routes [expect 200] failed: " + failureReason + " (" + errorType + ")", t);
                    }
                } else {
                    // ⏭️ SKIP: Clean skip reporting with proper Allure status
                    System.out.println("⏭️ SKIPPING: Root 1: ts-route-service GET /api/v1/routeservice/routes [expect 200] - " + skipReason);
                    stepResults.put(1, false);
            ...
            if (stepException instanceof RuntimeException && stepException.getMessage().startsWith("Step failed:")) {
                ...
                System.out.println("⚠️ Step wrapper failed for Root 1: ts-route-service GET /api/v1/routeservice/routes [expect 200]: " + stepException.getMessage());
                stepResults.put(1, false);
            }
        }
