...
public class get_api_v1_consignservice_consigns_order_2201 {
    ...
    public void test_negative_GET_2201_8() throws Exception {
        ...
        Allure.addAttachment("📝 Original Failure", "text/plain", "Original Status: 200\nOriginal Response: {\"status\":0,\"msg\":\"No Content according to consignee\",\"data\":null}\nEnhanced Parameters: {status=-1}");
        ...
        es.us.isa.restest.enhancer.TestResultCapture.setTestMetadata(
            "/api/v1/consignservice/consigns/order/", "GET", 
        ...
        // Step 1: ts-consign-service GET /api/v1/consignservice/consigns/order/ [expect not 200]
        ...
        try {
            Allure.step("Step 1: ts-consign-service GET /api/v1/consignservice/consigns/order/ [expect not 200]", () -> {
                Allure.parameter("🏢 Service", "ts-consign-service");
                Allure.parameter("📡 HTTP Method", "GET");
                Allure.parameter("🔗 Endpoint", "/api/v1/consignservice/consigns/order/");
                Allure.parameter("🎯 Expected Status", 200);
                ...
                Allure.description("🎯 **Testing**: ts-consign-service\n" +
                                 "📡 **Method**: GET\n" +
                                 "🔗 **Path**: /api/v1/consignservice/consigns/order/\n" +
                                 "🎯 **Expected**: 200\n" +
                ...
                if (!shouldSkip) {
                    System.out.println("▶️ EXECUTING: Step 1: ts-consign-service GET /api/v1/consignservice/consigns/order/ [expect not 200] (dependency analysis passed)");
                    ...
                    try {
                        ...
                        stepResponse1 = req.when().get("/api/v1/consignservice/consigns/order/")
                               .then().log().ifValidationFails()
                               .extract().response();
                        ...
                        System.out.println("✅ Step 1: ts-consign-service GET /api/v1/consignservice/consigns/order/ [expect not 200] - SUCCESS");
                        ...
                        try {
                            ...
                            attachJaegerTrace("ts-consign-service", "GET", "/api/v1/consignservice/consigns/order/", requestStartMicros, allStepParameters, false);
                        } catch (Exception e) {
                            Allure.parameter("🎯 Result", "✅ SUCCESS (response capture failed)");
                        }
                    } catch (Throwable t) {
                        stepResults.put(1, false);
                        System.out.println("❌ Step 1: ts-consign-service GET /api/v1/consignservice/consigns/order/ [expect not 200] - FAILED: " + t.getMessage());
                        ...
                        Allure.parameter("🔗 Failed Endpoint", "/api/v1/consignservice/consigns/order/");
                        ...
                        errorDetails.append("=====================================\n\n");
                        errorDetails.append("📋 Step: Step 1: ts-consign-service GET /api/v1/consignservice/consigns/order/ [expect not 200]\n");
                        errorDetails.append("🏢 Service: ts-consign-service\n");
                        errorDetails.append("📡 Method: GET\n");
                        errorDetails.append("🔗 Endpoint: /api/v1/consignservice/consigns/order/\n");
                        ...
                        // ⏱️ Wait longer for trace propagation to Jaeger (increased delay)
                        try { Thread.sleep(3000); } catch (InterruptedException ie) { Thread.currentThread().interrupt(); }
                        attachJaegerTrace("ts-consign-service", "GET", "/api/v1/consignservice/consigns/order/", requestStartMicros, allStepParameters, true);
                        
                        // 🔥 CRITICAL: Throw exception to mark step as FAILED (red arrow) in Allure
                        throw new RuntimeException("Step 1: ts-consign-service GET /api/v1/consignservice/consigns/order/ [expect not 200] failed: " + failureReason + " (" + errorType + ")", t);
                    }
                } else {
                    // ⏭️ SKIP: Clean skip reporting with proper Allure status
                    System.out.println("⏭️ SKIPPING: Step 1: ts-consign-service GET /api/v1/consignservice/consigns/order/ [expect not 200] - " + skipReason);
                    stepResults.put(1, false);
            ...
            if (stepException instanceof RuntimeException && stepException.getMessage().startsWith("Step failed:")) {
                ...
                System.out.println("⚠️ Step wrapper failed for Step 1: ts-consign-service GET /api/v1/consignservice/consigns/order/ [expect not 200]: " + stepException.getMessage());
                stepResults.put(1, false);
            }
        }
