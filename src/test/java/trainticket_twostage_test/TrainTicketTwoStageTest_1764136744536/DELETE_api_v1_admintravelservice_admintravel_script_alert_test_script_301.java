...
public class DELETE_api_v1_admintravelservice_admintravel_script_alert_test_script_301 {
    ...
    @Test
    public void test_negative_DELETE_301_38() throws Exception {
        // Record test execution for fault detection tracking
        es.us.isa.restest.analysis.FaultDetectionTracker.getInstance()
            .recordTestCase(this.getClass().getName(), "test_negative_DELETE_301_38");
        ...
        // Step 1: ts-admin-travel-service DELETE /api/v1/admintravelservice/admintravel/<script>alert('test')</script> (expect != 200 (any error))
        ...
        try {
            Allure.step("Step 1: ts-admin-travel-service DELETE /api/v1/admintravelservice/admintravel/<script>alert('test')</script> (expect != 200 (any error))", () -> {
                ...
                if (!shouldSkip) {
                    System.out.println("▶️ EXECUTING: Step 1: ts-admin-travel-service DELETE /api/v1/admintravelservice/admintravel/<script>alert('test')</script> (expect != 200 (any error)) (dependency analysis passed)");
                    ...
                    try {
                                ...
                                if (dataObj.optBoolean("injected", false)) {
                                    ...
                                    if (!detectedFaultName.isEmpty()) {
                                        es.us.isa.restest.analysis.FaultDetectionTracker.getInstance().recordDetectedFault(
                                            ...
                                            "test_negative_DELETE_301_38",
                                            System.currentTimeMillis(),
                                            faultCheckBody
                                        );
                                        System.out.println("🔍 FAULT DETECTED: " + detectedFaultName + " in test: test_negative_DELETE_301_38");
                                    }
                                }
                            }
                        ...
                        System.out.println("✅ Step 1: ts-admin-travel-service DELETE /api/v1/admintravelservice/admintravel/<script>alert('test')</script> (expect != 200 (any error)) - SUCCESS");
                        ...
                        System.out.println("❌ Step 1: ts-admin-travel-service DELETE /api/v1/admintravelservice/admintravel/<script>alert('test')</script> (expect != 200 (any error)) - FAILED: " + t.getMessage());
                        ...
                        errorDetails.append("=====================================\n\n");
                        errorDetails.append("📋 Step: Step 1: ts-admin-travel-service DELETE /api/v1/admintravelservice/admintravel/<script>alert('test')</script> (expect != 200 (any error))\n");
                        ...
                        throw new RuntimeException("Step 1: ts-admin-travel-service DELETE /api/v1/admintravelservice/admintravel/<script>alert('test')</script> (expect != 200 (any error)) failed: " + failureReason + " (" + errorType + ")", t);
                    }
                } else {
                    // ⏭️ SKIP: Clean skip reporting with proper Allure status
                    System.out.println("⏭️ SKIPPING: Step 1: ts-admin-travel-service DELETE /api/v1/admintravelservice/admintravel/<script>alert('test')</script> (expect != 200 (any error)) - " + skipReason);
                    stepResults.put(1, false);
            ...
            if (stepException instanceof RuntimeException && stepException.getMessage().startsWith("Step failed:")) {
                ...
                System.out.println("⚠️ Step wrapper failed for Step 1: ts-admin-travel-service DELETE /api/v1/admintravelservice/admintravel/<script>alert('test')</script> (expect != 200 (any error)): " + stepException.getMessage());
                stepResults.put(1, false);
            }
        }
        ...
        Allure.label("story", "test_negative_DELETE_301_38");
        Allure.description("Microservice test scenario with " + totalSteps + " steps.");
        
        System.out.println("=== SCENARIO RESULT ===");
        System.out.println("Scenario: test_negative_DELETE_301_38");
