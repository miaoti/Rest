...
public class get_api_v1_executeservice_execute_collected_a_801 {
    ...
    public void test_negative_GET_801_56() throws Exception {
        ...
        // Step 1: ts-execute-service GET /api/v1/executeservice/execute/collected/a [expect not 200]
        ...
        try {
            Allure.step("Step 1: ts-execute-service GET /api/v1/executeservice/execute/collected/a [expect not 200]", () -> {
                Allure.parameter("🏢 Service", "ts-execute-service");
                Allure.parameter("📡 HTTP Method", "GET");
                Allure.parameter("🔗 Endpoint", "/api/v1/executeservice/execute/collected/a");
                Allure.parameter("🎯 Expected Status", 200);
                ...
                Allure.description("🎯 **Testing**: ts-execute-service\n" +
                                 "📡 **Method**: GET\n" +
                                 "🔗 **Path**: /api/v1/executeservice/execute/collected/a\n" +
                                 "🎯 **Expected**: 200\n" +
                ...
                if (!shouldSkip) {
                    System.out.println("▶️ EXECUTING: Step 1: ts-execute-service GET /api/v1/executeservice/execute/collected/a [expect not 200] (dependency analysis passed)");
                    ...
                    try {
                        ...
                        stepResponse1 = req.when().get("/api/v1/executeservice/execute/collected/a")
                               .then().log().ifValidationFails()
                               .extract().response();
                            ...
                            try {
                                ...
                                if (softErrorCacheEnabled) {
                                    ...
                                    validationResult = llmValidator.validateNegativeResponseWithCache(
                                        ...
                                        "ts-execute-service",
                                        "GET",
                                        "/api/v1/executeservice/execute/collected/a",
                                        invalidParams,
                                        cache
                                    );
                                } else {
                                    ...
                                    if (dataObj.optBoolean("injected", false)) {
                                        ...
                                        if (!detectedFaultName.isEmpty()) {
                                            es.us.isa.restest.analysis.FaultDetectionTracker.getInstance().recordDetectedFault(
                                                ...
                                                "test_negative_GET_801_56",
                                                System.currentTimeMillis(),
                                                faultCheckBody
                                            );
                                            System.out.println("🔍 FAULT DETECTED: " + detectedFaultName + " in test: test_negative_GET_801_56");
                                        }
                                    }
                                }
                            }
                        ...
                        System.out.println("✅ Step 1: ts-execute-service GET /api/v1/executeservice/execute/collected/a [expect not 200] - SUCCESS");
                        ...
                        try {
                            ...
                            attachJaegerTrace("ts-execute-service", "GET", "/api/v1/executeservice/execute/collected/a", requestStartMicros, allStepParameters, false);
                        } catch (Exception e) {
                            Allure.parameter("🎯 Result", "✅ SUCCESS (response capture failed)");
                        }
                    } catch (Throwable t) {
                        stepResults.put(1, false);
                        System.out.println("❌ Step 1: ts-execute-service GET /api/v1/executeservice/execute/collected/a [expect not 200] - FAILED: " + t.getMessage());
                                    ...
                                    if (dataValue instanceof org.json.JSONObject) {
                                        ...
                                        if (dataObj.optBoolean("injected", false)) {
                                            ...
                                            if (!detectedFaultName.isEmpty()) {
                                                ...
                                                System.out.println("🔍 FAULT DETECTED (in failed test): " + detectedFaultName + " in test: test_negative_GET_801_56");
                                            }
                                        }
                                    }
                                }
                        ...
                        Allure.parameter("🔗 Failed Endpoint", "/api/v1/executeservice/execute/collected/a");
                        ...
                        errorDetails.append("=====================================\n\n");
                        errorDetails.append("📋 Step: Step 1: ts-execute-service GET /api/v1/executeservice/execute/collected/a [expect not 200]\n");
                        errorDetails.append("🏢 Service: ts-execute-service\n");
                        errorDetails.append("📡 Method: GET\n");
                        errorDetails.append("🔗 Endpoint: /api/v1/executeservice/execute/collected/a\n");
                        ...
                        // ⏱️ Wait longer for trace propagation to Jaeger (increased delay)
                        try { Thread.sleep(3000); } catch (InterruptedException ie) { Thread.currentThread().interrupt(); }
                        attachJaegerTrace("ts-execute-service", "GET", "/api/v1/executeservice/execute/collected/a", requestStartMicros, allStepParameters, true);
                        
                        // 🔥 CRITICAL: Throw exception to mark step as FAILED (red arrow) in Allure
                        throw new RuntimeException("Step 1: ts-execute-service GET /api/v1/executeservice/execute/collected/a [expect not 200] failed: " + failureReason + " (" + errorType + ")", t);
                    }
                } else {
                    // ⏭️ SKIP: Clean skip reporting with proper Allure status
                    System.out.println("⏭️ SKIPPING: Step 1: ts-execute-service GET /api/v1/executeservice/execute/collected/a [expect not 200] - " + skipReason);
                    stepResults.put(1, false);
            ...
            if (stepException instanceof RuntimeException && stepException.getMessage().startsWith("Step failed:")) {
                ...
                System.out.println("⚠️ Step wrapper failed for Step 1: ts-execute-service GET /api/v1/executeservice/execute/collected/a [expect not 200]: " + stepException.getMessage());
                stepResults.put(1, false);
            }
        }
        ...
        Allure.label("story", "test_negative_GET_801_56");
        Allure.description("Microservice test scenario with " + totalSteps + " steps.");
        
        System.out.println("=== SCENARIO RESULT ===");
        System.out.println("Scenario: test_negative_GET_801_56");
