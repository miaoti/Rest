...
public class RESTAssuredWriter implements IWriter {
	...
	public RESTAssuredWriter(String specPath, String testConfPath, String targetDirJava, String className, String packageName, String baseURI, Boolean logToFile) {
		...
		this.testConf = buildEmptyConf();          // ← always have *something*

		if (testConfPath != null) {
			try {
				this.testConf = loadConfiguration(testConfPath, spec);
			} catch (Exception ex) {
				logger.warn("Could not parse testConf '{}': {} – using empty configuration instead",
						testConfPath, ex.getMessage());
			}
		}
	...
	// helper
	private TestConfigurationObject buildEmptyConf() {
		TestConfiguration tc = new TestConfiguration();
		tc.setOperations(new ArrayList<>());   // avoid NPEs in visitors

		TestConfigurationObject tco = new TestConfigurationObject();
		tco.setTestConfiguration(tc);
		tco.setAuth(new Auth());               // empty auth block
		return tco;
	}
...
//		if (OAIValidation)
	...
	private String generateHTTPRequest(TestCase t) {
		...
		// next lines: chain .statusCode(...) if expectedResponse is set
		content += "\n\t\t\tresponse.then()";
		if (t.getExpectedResponse() != null && t.getEnableOracles()) {
			content += ".statusCode(" + t.getExpectedResponse() + ")";
		}
		content += ";\n";
