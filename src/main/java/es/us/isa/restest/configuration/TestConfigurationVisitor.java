...
public class TestConfigurationVisitor {
	...
	public static boolean hasStatefulGenerators(TestConfigurationObject conf) {
		if (conf == null ||
				conf.getTestConfiguration() == null ||
				conf.getTestConfiguration().getOperations() == null) {
			return false;
		}
		return conf.getTestConfiguration()
				.getOperations()
				.stream()
				.anyMatch(TestConfigurationVisitor::hasStatefulGenerators);
	}
