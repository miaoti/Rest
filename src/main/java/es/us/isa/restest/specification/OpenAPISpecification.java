...
public class OpenAPISpecification {
	...
	public List<OpenAPIOperation> getAllOperations() {
		List<OpenAPIOperation> ops = new ArrayList<>();
		if (specification.getPaths() == null) {
			return ops;
		}
		for (Entry<String, PathItem> entry : specification.getPaths().entrySet()) {
			String path = entry.getKey();
			PathItem pi = entry.getValue();
			// readOperationsMap() gives you a Map<HttpMethod,Operation>
			if (pi.readOperationsMap() != null) {
				for (Entry<PathItem.HttpMethod, io.swagger.v3.oas.models.Operation> m : pi.readOperationsMap().entrySet()) {
					ops.add(new OpenAPIOperation(path, m.getKey().name().toLowerCase(), m.getValue()));
				}
			}
		}
		return ops;
