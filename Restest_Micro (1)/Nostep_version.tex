...
propagate as the system architect intended?  Did the response body's
semantic envelope flag a domain-level rejection that the HTTP status
masked?}  Each of those questions
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Trace Shape Oracle (new).}~A learner--oracle pair that
        ...
        produces three families of checkable per-API invariants---span-
        tree shape, status propagation, and response envelope---then
        verifies each subsequent live trace against them.  The three
        families are chosen so the oracle remains effective on trace
        corpora whose request and response bodies have been redacted
        upstream for GDPR/HIPAA compliance; this is the deployment
        regime in which body-inspection oracles such as
        LogiAgent~\cite{zhang2025logiagent}'s Response Validator cannot
        operate.  The legacy soft-error rule cache becomes one invariant
...
pattern}.  Section~\ref{ssec:tso} below identifies three families of
...
family among three---so the contribution is the trace lift, not the
LLM-judged response check.  We further restrict the chosen families to
those that remain meaningful on PII-stripped trace corpora, a regime in
which LogiAgent's body-reading Validator cannot operate at all.
...
The pipeline comprises seven concept-level subsystems. The first five are inherited from the trace-driven generator on which we build; the last two are the contributions of this paper.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Trace Shape Oracle}~(new)
        ...
        A learner-oracle pair that ingests a small seed corpus of known-good traces, produces three families of per-API invariants---on the span-tree shape, the status-code propagation pattern, and the response envelope---and checks each subsequent live trace against them at first-class assertion strength.  The three families operate only on trace metadata that survives the body-redaction common to GDPR/HIPAA-compliant OTel exporters.
...
The Trace Shape Oracle promotes the trace from diagnostic side-channel to checkable assertion. An off-line learner reads a small labelled corpus of known-good traces and condenses each root API's induced subtree into three families of per-API invariants. An on-line oracle consumes the freshly-pulled trace of each test step and produces a structured verdict against those invariants. Soft-error detection becomes one of the three families rather than a stand-alone subsystem.

\paragraph{Why three families, not more.}
The three families chosen below (span-tree shape, status propagation, response envelope) share two properties that motivate their selection.  First, each one is a \emph{functional-correctness} invariant: a violation indicates either a missing service interaction or a domain-level rejection, not a performance phenomenon.  Second, each one operates on trace metadata that survives the request- and response-body redaction common to GDPR- and HIPAA-compliant OpenTelemetry exporters in production: span structure, HTTP and OTel status codes, and (for the response-envelope family) a one-shot language-model classification of the response body that is invoked at most twice per distinct root API and then cached.  We deliberately omit a timing-envelope family from the contribution because microservice end-to-end latency is dominated by cold-start, garbage-collection, and cluster-load variance whose stable characterisation falls within the scope of dedicated trace anomaly detection in production diagnostics (TraceRCA~\cite{li2021tracerca}, MicroHECL~\cite{liu2021microhecl}, ImpactTracer~\cite{xie2023impacttracer}) rather than functional testing; including such a family in a testing oracle would conflate test verdicts with monitoring alerts and require a much larger seed corpus for stable percentile estimation.
...
\]
where $b \in \{\textsc{pass}, \textsc{fail}, \textsc{warn}\}$ is the verdict bit and $V$ is a list of structured violations identifying the offending span(s) and the failed sub-condition. The three families share this contract.
