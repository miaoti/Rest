...
\textsc{MIST} is structured so that its contribution can be evaluated, vendored, or replaced without coupling to any particular host framework. Conceptually, the tool decomposes into three layers. A \emph{core library} carries every generation, oracle, and registry artefact that is novel to this work---including the two contributions described in this paper. A \emph{language-model substrate} exposes a single abstraction for prompting and caching, so that backend choice (local or hosted) and reproducibility under a fixed seed are policy decisions rather than code changes. An \emph{adapter layer} bridges the core to an existing OpenAPI-driven generator framework, reusing its specification parser, test-case data model, and JUnit/REST-Assured writer. The core library has no compile-time dependency on the adapter or on the host framework; the language-model substrate depends on neither. A single entry point invokes the tool from the command line.\footnote{The implementation ships as four Maven modules---\texttt{mist-core} (core library), \texttt{mist-llm} (language-model substrate), \texttt{mist-restest-adapter} (adapter to RESTest~\cite{martin2019restest}), and \texttt{mist-cli} (entry point)---in the repository at \url{https://github.com/miaoti/Rest}. A SHA-256-keyed on-disk cache makes language-model calls replayable under a fixed seed.}
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Trace Shape Oracle}~(new)
        ...
        A learner-oracle pair that ingests a small seed corpus of known-good traces, produces four families of per-API invariants---on the span-tree shape, the status-code propagation pattern, the timing envelope, and the response envelope---and checks each subsequent live trace against them at first-class assertion strength.
...
The Trace Shape Oracle promotes the trace from diagnostic side-channel to checkable assertion. An off-line learner reads a small labelled corpus of known-good traces and condenses each root API's induced subtree into four families of per-API invariants. An on-line oracle consumes the freshly-pulled trace of each test step and produces a structured verdict against those invariants. Soft-error detection becomes one of the four families rather than a stand-alone subsystem.
...
\]
where $b \in \{\textsc{pass}, \textsc{fail}, \textsc{warn}\}$ is the verdict bit and $V$ is a list of structured violations identifying the offending span(s) and the failed sub-condition. The four families share this contract.
...
\paragraph{Timing envelope.}
For each root API, the learner records total-duration and per-span percentiles $(p_{50}, p_{95}, p_{99})$ from known-good traces. The oracle flags a live trace whose total duration exceeds the learned $p_{99}$, or whose individual span exceeds its position's learned $p_{99}$ by a factor of two or more. Timing invariants are intrinsically noisy on real microservice workloads---containers cold-start, GC pauses, network jitter---so this family ships at warning severity by default; operators escalate it to a hard verdict only on services with strict latency SLOs.
