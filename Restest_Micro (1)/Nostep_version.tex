...
Our system implements the \emph{Multi-Root Sequence model}: on a successful merge, the \textproc{MergeWith} method \emph{promotes} the consumer root step to a new top-level root within the producer's \textsc{WorkflowScenario}. Two metadata fields are set on the promoted step: \texttt{mergedRoot = true} and \texttt{producerRootIndex}, which records the 1-based index of the producer root that justified the merge. The result is a single \textsc{WorkflowScenario} whose \texttt{rootSteps} list contains multiple independent Root APIs in causal order, e.g., \textit{Root~1: POST /api/v1/trips}, followed by \textit{Root~2: POST /api/v1/orders} (which depends on the \texttt{tripId} produced by Root~1).
...
To support Priority~0b JIT binding, we introduce the \emph{Semantic Dependency Registry}, a lightweight schema-driven dictionary built once at startup from the loaded \texttt{TestConfigurationObject} instances (parsed from the multi-service YAML). The registry operates in two passes: (i)~\emph{producer registration}---each POST or PUT operation on an entity resource (e.g., \texttt{POST /api/v1/orderservice/order}) is registered as a \emph{strong producer} for an ID-like field derived from the resource noun (e.g., \texttt{orderId}); operations that carry an ID-like path parameter are registered as \emph{weak producers}; (ii)~\emph{consumer indexing}---for every parameter whose name ends with \texttt{id}, \texttt{Id}, or \texttt{uuid} in any operation, the registry finds the best matching producer (preferring cross-service strong producers) and stores a \texttt{ProducerBinding} record containing the producer's service name, API key, and a heuristic JSON path (e.g., \texttt{data.id}). At generation time, \textproc{FindProducer}(\textit{consumerApiKey}, \textit{paramName}) normalises the parameter name to a noun stem (e.g., \texttt{orderId}~$\to$~\texttt{order}), looks up the consumer index, and returns the binding or null. The generator then scans the already-added steps of the current \textsc{MultiServiceTestCase} to confirm that the identified producer API is actually present at some step~$k < N$; only if confirmed does it wire the dependency, ensuring that the registry's heuristic recommendation is grounded in the actual scenario topology.
